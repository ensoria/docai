...
function validateSources(documentSet, root, markdown) {
...
export function validateCoreDocumentSet(documentSet) {
  ...
  const sources = structure.diagnostics.length === 0
    ? validateSources(documentSet, root, scanned.value)
  ...
  return {
    diagnostics: [...profile.diagnostics, ...structure.diagnostics, ...sources.diagnostics],
    facts
  };
}
