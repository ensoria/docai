...
`PUBLICATION.json` records the trusted out-of-band Compatibility Core scope, exact projection-manifest digest, and adapter mapping identities and versions. `PUBLICATION-REVIEW.md` records the separate format-compliance, contract-completeness, reader-relative-readiness, and stop-rule judgments. Passing the checker validates the metadata binding; it does not replace the human review. The root README alone declares the repository publication label, and that label is limited to this reviewed Compatibility Core scope.
