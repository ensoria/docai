> docai-messaging: 0.17.1 | profile: full | perspective: storefront | coverage: complete | knowledge: complete | source_refs: all

# Messaging Conventions

## Environments

none

## Protocols and Bindings

none

## Authentication

none

## Connection and Session

none

## Serialization

none

## Message Envelope

none

## Delivery Semantics

none

## Idempotency and Deduplication

none

## Ordering

none

## Error Handling

none

## Request-Reply

none

## Schema Evolution

none

## Data Representation

| Format | Role | Meaning |
|---|---|---|
| "date-time" | annotation | Preserve RFC 3339 date-time representation intent without adding validation or construction behavior. |
| "int32" | constraint | Accept signed 32-bit integers and construct and validate them without narrowing. |

## Empty and Omitted Values

none

## Rate Limits and Quotas

none

> docai-identity: set_id: b32:r6s2abwqhnffhouzsfivx7uv6u | projection_id: b32:2su6l5snggpayed76bebjwuzuy
