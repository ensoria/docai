...
```markdown
> docai-messaging: 0.8.0 | profile: full | perspective: storefront-service | coverage: complete | knowledge: complete | source_refs: storefront-api, storefront-conventions
...
```markdown
> docai-messaging: 0.8.0 | profile: full | perspective: storefront-service | coverage: complete | knowledge: complete | source_refs: all
...
- Under the registered AsyncAPI 3.1.0 adapter, an omitted operation `messages` property selects every message from the referenced channel. A non-empty property selects its explicit referenced-channel subset. Operation `messages: []`, or omission when the referenced channel contains no messages, resolves to a known zero-message operation outside the 0.8 operation model and uses the localized `unsupported` Unprojected Operations form above; do not invent a primary message.
- Under the registered AsyncAPI 3.0.0 adapter, a non-empty operation `messages` property selects its explicit referenced-channel subset, and `messages: []` uses the same known zero-message `unsupported` form. The AsyncAPI 3.0.0 specification does not itself establish the selected set when the property is omitted. Unless a separate authoritative operation-message-selection input satisfying §3.6 supplies a non-empty set, omit the operation and use the `unknown` Unprojected Operations form naming that required input. An authoritative selection of an empty set uses `unsupported`, not `unknown`. The selection input has its own Sources row, participates in precedence and conflict handling, contributes its source ID to affected files, and participates in `projection_id` under §3.6.
...
A reader must not interpret an unknown profile as `full`. It reports the unsupported profile unless the unknown-profile INDEX provides a `Full set:` link whose target uses a supported full profile with the same `projection_id` and `projection_digest`; in that case it uses the full set. This fallback rule is part of the 0.8 contract so a future optional runtime profile can remain safely ignorable after 1.0.
...
- Use the optional one-line marker `**field_defaults**: <column>=<value>` immediately before a compact header or payload-field table to omit one or more uniform columns. Separate multiple defaults with ` | `. Valid defaults are `Required=yes|no`, `Presence=always|optional`, `Nullable=yes|no`, and `Meaning=none`. `Meaning=none` is a sentinel that reconstructs each omitted `Meaning` cell as the empty string; it never reconstructs the literal structural value `none`. It applies only when the full table's final column is exactly `Meaning` and every omitted cell is empty. Every other default reconstructs the named column with its literal marker value. A default is valid only when the named column exists in the table's full-profile form and every row reconstructs to that value. `Required=conditional`, conditional Presence, and any column containing `unknown` cannot be defaulted. Reconstruct the logical full-profile columns in their standard positions before applying any other validation rule. Emit the marker only when the marker plus shorter table costs fewer measured tokens than the unreduced table.
...
DocAI Messaging 0.8 directly defines the following projection rules:
...
| `if` / `then` / `else`, `not`, `patternProperties`, `propertyNames`, tuple-validation arrays, `contains`, `dependencies`, `dependentRequired`, `dependentSchemas`, and `unevaluatedProperties` / `unevaluatedItems` | Outside the 0.8 directly representable scope; use the smallest applicable `**unsupported**:` form |
...
Wire-encoding projection is default-deny independently of schema projection. DocAI Messaging 0.8 directly registers a JSON wire adapter only for the exact media type `application/json` without parameters and for parameterless media types whose lowercase subtype ends in `+json`. That adapter encodes the documented decoded value as UTF-8 JSON, renders its example in a `json` fenced block, and uses the logical schema projection for the field table. Every other structured, textual, or parameterized media type requires a published exact-version wire adapter that defines byte encoding, character encoding when applicable, media-type-parameter semantics, decoded-value model, example rendering, and composition with the selected schema adapter. A representation is complete only when both adapters exist and their composition is defined without information loss; otherwise replace the smallest affected payload representation with `**unsupported**:`. A familiar suffix, a source example, or a schema that resembles JSON is not by itself a wire adapter.
...
`````markdown
> docai-messaging: 0.8.0 | profile: full | perspective: storefront-service | coverage: complete | knowledge: complete | source_refs: partner-guide
...
| AsyncAPI 3.1.0 operation `messages` | A non-empty list selects that referenced-channel subset; omission selects every referenced-channel message; `[]` and omission against an empty channel are zero-message operations reported under `Unprojected Operations` as unsupported in 0.8 |
| AsyncAPI 3.0.0 operation `messages` | A non-empty list selects that referenced-channel subset; omission is unknown unless a separate authoritative operation-message-selection input establishes a non-empty set; `[]` or an authoritative empty set is unsupported in 0.8 |
| AsyncAPI 3.0.0 / 3.1.0 reply `messages` | A non-empty list selects its referenced reply messages; omission is unknown regardless of referenced-channel message count unless a separate authoritative reply-message-selection input establishes a non-empty set; `[]` or an authoritative empty set is unsupported in 0.8 |
...
- [ ] A flat INDEX groups projected operations into one `###` subsection per channel file and fills `Action`, `Channel`, `Operation`, `Message`, `Task`, `Summary`, `Required context`, and `Supplemental context`, or writes `none` for an empty set; context paths are unique, ASCII-ordered, use exact `, ` separators, and target only their 0.8-eligible workflow or Reference Material grammars; required context is loaded unconditionally, supplemental context contains no required fact, and the `Message` cell lists lexical primary names followed only by lexical `reply:<name>` entries from an expanded Reply
...
- [ ] Every source operation with a missing or unrepresentable routing fact, or another fact that prevents any complete definition, is absent from normal rows and has one marker per applicable completeness dimension in direct or sharded `Unprojected Operations`; non-routing facts use their section-local forms when possible; AsyncAPI 3.1.0 omitted operation `messages` selects all channel messages but an empty channel result is unsupported, while AsyncAPI 3.0.0 omission is unknown without a separate authoritative non-empty selection; every other explicit or authoritative empty set is unsupported; and reply-only message-set or identity failures use the whole-Reply forms without removing the primary operation
...
- [ ] Root and shard INDEX boundaries, operation headings, channel addresses, message names, media types, file paths, profile-link paths, table escape parity and cell normalization, field paths, dynamically bounded fences, hash inputs, and the type grammar follow §3.5; every source-prose line is neutralized when it could parse as any standard or `x-` structural form, including metadata, identity, profile-link, Behavior or Reply key, and collapsed-`none` forms
- [ ] Schema projection is default-deny under §3.6: every represented source feature has a defined direct or published exact-version adapter rule; constraint fragments preserve exact values in canonical order; JSON Schema Draft 07 `default` becomes non-behavioral `default_annotation`, while AsyncAPI Schema Object `default` becomes direction-specific effective `default`; neither changes Required, Presence, or Nullable; Server Variable and Channel Parameter defaults retain their separate substitution or send behavior; and every unlisted semantic feature uses the smallest applicable `unsupported` form
...
- [ ] Every non-empty payload states `**payload_required**: yes|no|unknown`(send-side) or `**payload_presence**: always|optional|<exact condition>|unknown`(receive-side); every expanded non-raw representation starts with `**media_type**:`, followed by `**payload_nullable**:`, a concrete example, and its field table; opaque raw binary instead uses the §4.1 media-type-plus-prose form; each concrete media type appears at most once, with alternatives represented as variants or unsupported
- [ ] Every example field, including containers, has a corresponding field-table row except the root-object `$` exception; generated or sanitized examples use constraint-valid synthetic values, satisfy every machine-verifiable source constraint or carry the required unknown indication, are realistic and reused consistently, and contain no real credential, real personal or regulated data, or confidential production value
