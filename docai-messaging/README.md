...
```markdown
> docai-messaging: 0.7.0 | profile: full | perspective: storefront-service | coverage: complete | knowledge: complete | source_refs: storefront-api, storefront-conventions
...
The default Compatibility Core contains the `full` profile; opening metadata and final identity trailers; the direct or sharded structured Sources manifest; flat and hierarchical operation routing including required and supplemental context columns; direct or sharded Unprojected Operations routing; the exact-version AsyncAPI 3.0.0 and 3.1.0 operation- and reply-message selection rules; the JSON Schema Draft 07 and AsyncAPI Schema Object default rules; the §3.7 instruction trust boundary; whole-file CONVENTIONS loading without selective convention loading; ordinary operation, Message, Reply, and Failure Handling structures including receive-side `optional`; canonical syntax; and `unknown` / `unsupported` signaling. A Core reader ignores the optional operation-table `Conventions` column and loads all of `CONVENTIONS.md`. When a Core operation's required context points to an advanced structure outside the Core, the reader reports that it cannot complete that operation within the Core.
...
```markdown
> docai-messaging: 0.7.0 | profile: full | perspective: storefront-service | coverage: complete | knowledge: complete | source_refs: all
...
- Under the registered AsyncAPI 3.1.0 adapter, an omitted operation `messages` property selects every message from the referenced channel. A non-empty property selects its explicit referenced-channel subset. Operation `messages: []`, or omission when the referenced channel contains no messages, resolves to a known zero-message operation outside the 0.7 operation model and uses the localized `unsupported` Unprojected Operations form above; do not invent a primary message.
...
A reader must not interpret an unknown profile as `full`. It reports the unsupported profile unless the unknown-profile INDEX provides a `Full set:` link whose target uses a supported full profile with the same `projection_id` and `projection_digest`; in that case it uses the full set. This fallback rule is part of the 0.7 contract so a future optional runtime profile can remain safely ignorable after 1.0.
...
DocAI Messaging 0.7 directly defines the following projection rules:
...
| `if` / `then` / `else`, `not`, `patternProperties`, `propertyNames`, tuple-validation arrays, `contains`, `dependencies`, `dependentRequired`, `dependentSchemas`, and `unevaluatedProperties` / `unevaluatedItems` | Outside the 0.7 directly representable scope; use the smallest applicable `**unsupported**:` form |
...
`````markdown
> docai-messaging: 0.7.0 | profile: full | perspective: storefront-service | coverage: complete | knowledge: complete | source_refs: partner-guide
...
- Resolve the reply message set before emitting the Reply keys or adding `reply:` entries to the INDEX `Message` cell. Under the registered AsyncAPI 3.1.0 adapter, a non-empty `reply.messages` property selects the message objects referenced by those entries, subject to §3.6. When that property is omitted and the referenced reply channel contains exactly one message, select that message. When the referenced channel contains zero or multiple messages, or when no reply channel is referenced, omission does not establish the reply message set; use the whole-section `unknown` form below unless a separate authoritative reply-message-selection input satisfying §3.6 supplies a non-empty set. Under the registered AsyncAPI 3.0.0 adapter, omission does not establish the set regardless of whether a reply channel is referenced or how many messages it contains and requires the same authoritative input. Other versions follow their published exact-version adapter.
- An explicit `reply.messages: []` or an authoritative reply-message selection of an empty set is a known zero-message reply outside the 0.7 Reply model. Under `### Reply`, emit only `**unsupported**: replaces Reply: zero-message reply <source location>` and apply `coverage: requires-source`; retain the primary operation and omit reply entries from its INDEX `Message` cell. Any separate authoritative reply-message-selection input has its own Sources row and follows the precedence, conflict, source-reference, and `projection_id` rules of §3.6.
- When authoritative inputs establish that a reply exists but do not establish a non-empty reply message set, write `unknown` followed immediately by `**unknown**: reply message set requires <expected authoritative input or source location>` under `### Reply`. Use the same whole-section form with `**unknown**: reply message identity requires <expected authoritative input, stable-name override, or source location>` when any selected reply message lacks a valid stable-name override, a usable source identifier, and stable identity for derived naming. In either case, do not emit the Reply keys, Channel, reply Message subsections, or `reply:` INDEX entries. These are section-local reply limitations and do not remove the otherwise projectable primary operation.
...
| AsyncAPI 3.1.0 operation `messages` | A non-empty list selects that referenced-channel subset; omission selects every referenced-channel message; `[]` and omission against an empty channel are zero-message operations reported under `Unprojected Operations` as unsupported in 0.7 |
| AsyncAPI 3.0.0 operation `messages` | A non-empty list selects that referenced-channel subset; omission is unknown unless a separate authoritative operation-message-selection input establishes a non-empty set; `[]` or an authoritative empty set is unsupported in 0.7 |
...
- [ ] A flat INDEX groups projected operations into one `###` subsection per channel file and fills `Action`, `Channel`, `Operation`, `Message`, `Task`, `Summary`, `Required context`, and `Supplemental context`, or writes `none` for an empty set; context paths are unique, ASCII-ordered, use exact `, ` separators, and target only their 0.7-eligible workflow or Reference Material grammars; required context is loaded unconditionally, supplemental context contains no required fact, and the `Message` cell lists lexical primary names followed only by lexical `reply:<name>` entries from an expanded Reply
...
- [ ] Every Reply is exactly `none`, a whole-section `unknown`, a replacement `unsupported`, or an expanded Reply; exact-version message selection occurs first, and omitted AsyncAPI 3.1.0 reply messages without exactly one referenced-channel message—including no referenced channel—remain unknown without a separate authoritative non-empty selection; only the expanded form begins with `channel` / `correlation` / `timeout`, contains Channel Parameters then Bindings, and then complete reply Message sections in the direction opposite to the operation action; unresolved and zero-message replies contribute no `reply:` INDEX entries, and embedded replies do not appear as separate operations
