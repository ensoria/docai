...
| Action | Channel | Message | Task | Summary | Also read |
|---|---|---|---|---|---|
| SEND | orders.commands | cancel-order | cancel order | Requests asynchronous cancellation; outcome arrives as a correlated reply | workflows/order-cancellation.md |
| RECEIVE | orders.events | order-shipped | track shipment | At-least-once delivery; deduplicate by `message_id` header | none |
...
- An operation heading is `## <ACTION> <address>` or `## <ACTION> <address> (<operation name>)`. `ACTION` is `SEND` or `RECEIVE`. The address is a non-empty channel address with no ASCII whitespace, using `{name}` for each address parameter; a parameter name must be non-empty and must not contain `/`, `{`, `}`, or ASCII whitespace. Literal `{` or `}` characters outside parameter delimiters cannot be represented. The optional operation name matches `[A-Za-z0-9._-]+` and is required when the same `ACTION` and address pair would otherwise appear more than once in the set. The complete heading text is unique within a set. If a source address or name cannot be represented by this grammar, the generator must not normalize it silently; use the applicable rules in §3.4.
...
**Heading(`## <ACTION> <address>`)**
- `ACTION` is `SEND` when the implemented application produces the message and `RECEIVE` when it consumes the message. Use the address grammar of §3.5, with `{name}` for address parameters. Append ` (<operation name>)` when needed for uniqueness(§3.5).
...
- `delivery`: the delivery guarantee and acknowledgement contract for this operation — for SEND, when a publish is considered durable and what an ambiguous outcome(timeout) means; for RECEIVE, the guarantee(at-most-once / at-least-once / exactly-once), when to acknowledge relative to processing, negative-acknowledgement behavior, and redelivery timing.
...
1. Load `INDEX.md`.
2. Select the operation row by `Task`, `Action`, `Channel`, `Message`, and `Summary`.
...
- [ ] INDEX.md begins with `# Messaging Index`, includes `Operations` then `Workflows` in order, groups operations into one `###` subsection per channel file, and fills `Action`, `Channel`, `Message`, `Task`, `Summary`, and `Also read` for every operation, or writes `none` for an empty section
...
- [ ] Operation headings, channel addresses, message names, media types, file paths, table syntax and cell normalization, field paths, and the type grammar follow §3.5; an operation whose action or address is absent or unrepresentable is not emitted
...
- [ ] The `Behavior` section uses `side_effects` / `idempotency` / `preconditions` / `authorization` / `delivery` / `ordering` in order, writes `none` only when non-applicability is established, and uses `unknown` plus its marker when a required fact is absent from authoritative inputs
