...
> **Approved design (`R8-CORE-025` catalog / required-workflow closure checkpoint):** missing catalog row は有効な別 format row を持つ独立 invalid document set とし、使用中の `int32` row だけが未解決になる単一 `DM-CONV-003` concern を固定する。required-workflow closure は同一の `format="uuid"` workflow を required context とする paired document set で、operation の selective Conventions selector が `Data Representation` を含む valid case と `Serialization` だけを含む invalid case を分離する。既存 context / readiness fixture は流用せず、readiness と format closure の回帰責務を混在させない。production validator と公開 document grammar は変更しない。

> **Implementation note (`R8-CORE-025` catalog / required-workflow closure checkpoint):** `conventions-format-catalog-missing-invalid` は有効な UUID catalog を保ったまま payload の `format="int32"` が一件も解決しない境界を単一 `DM-CONV-003` として固定した。`conventions-required-workflow-format-closure-valid` / `-invalid` は同一の required workflow path と UUID constraint fragmentを持ち、前者の exact `Data Representation` selector / catalog facts と後者の selector omission を versioned document-set evidence にした。Core corpus test は valid case の required context、selector、catalog row を literal assertion し、三 fixture を corpus 実行へ、二つの invalid fixture を既存 one-invalidity audit へ含める。影響として `R8-CORE-025` は `covered` となり、Core manifest は 199 cases / 148 invalid cases になる。production validator、公開 interface、document grammar、既存 source scenario semantics は変更しない。

> **Approved design (`R8-CORE-026` two-checkpoint closure):** source projection outcome と生成 Markdown の責務を分離する。第一 checkpoint は専用 `schema-example-source-scenario` evaluator で、直接表現可能な矛盾制約を実データから判定して exact payload-representation replacement を導出し、source example がない場合の production capability不足と、source example がある場合の validation capability不足を別の generation-failure outcome として固定する。同じ replacement line を持つ valid document set を追加し、schema emptiness 判定を generator capability判定より先に行う。第二 checkpoint は constraint-valid illustrative example と valid-example-values marker の valid / invalid document pair を追加し、marker が constraint-invalid example を正当化しないことを固定して `R8-CORE-026` を `covered` にする。既存 default / format 用 `schema-field-source-scenario` は拡張せず、example lifecycle の責務を混在させない。

> **Implementation note (`R8-CORE-026` unsatisfiable / generator-capability checkpoint):** `schema-example-projection-valid` は `minimum=10` / `maximum=5` の非nullable schema を、example production / validation capability がともに欠けても先に `replacement-unsupported` と判定し、`empty-schema-message 16:application/json` の exact replacement line を生成する。satisfiable schema は、source example がない production capability不足と、source example があるため production を必要としない validation capability不足を別々の generation failure にする。paired invalid scenario は validation incapability を empty-schema replacement と誤分類する一件だけを `DM-MSG-005` とし、diagnostic は case ID、message name、example valueを開示しない。`payload-unsatisfiable-constraints-replacement-valid` は導出された replacement line を既存 document validator で検証する。arbitrary-precision number comparison は `json-value.mjs` の共通 utilityへ移し、既存 field constraint validation と source evaluator の意味を一致させた。影響として Core manifest は 202 cases / 149 invalid cases となり、`R8-CORE-026` は valid-example-values marker pair 待ちの `partial` を維持する。公開 document grammar、既存 document validation semantics、default / format source scenario interface は変更しない。
...
> **Approved design (`R8-CORE-028` adapter-defined header-schema closure checkpoint):** 既存 `adapter-source-scenario` interfaceを維持し、header schema compositionを schema adapter resolution と header encoding/exposure resolutionの二段階で評価する。unregistered schema targetは、exact DocAI Messaging version / schema targetに一致し、logical types、constraint mapping、example-value projection、runtime schema resolutionをすべて定義するpublication-scope schema mappingを要求する。その後に同じschema targetとcompatibleでencoding / exposureを両方定義するexact header mappingを要求する。adapter-defined supported resultはschema rule / version / source provenanceを含み、既存direct-schema resultは変更しない。versioned valid scenarioはcomplete、missing / incomplete schema mapping、missing / incompatible header mappingを分離し、単一invalid scenarioはcomplete schema mappingとexposure欠落header ruleをsupportedと誤投影する一件を `DM-ADAPTER-003` とする。

> **Approved order change (`R8-CORE-028` exact-one publication-mapping hardening):** 実装reviewで共通 `publicationMapping()` が `.find()` によりduplicate exact mappingの先頭を採用する既存gapを確認した。README §3.6のexactly-one semanticsをadapter class間で一貫させるため、後続の全adapter-selection clauseに予定していたduplicate mapping対応の一部をユーザー承認により先行する。共通resolverはversion / class / targetに一致する候補をclass-specific completeness / compatibility判定より先に数え、候補がちょうど一件で、その唯一の候補がclass-specific predicateを満たす場合だけmappingを返す。versioned schema / header scenarioはcomplete / incompleteを混在させたduplicate schema mappingとcompatible / incompleteを混在させたduplicate header mappingを、既存wire scenarioはcomplete / incompleteを混在させたduplicate payload-wire mappingをunsupportedとして固定し、`DM-ADAPTER-004` unit evidenceはduplicate protocol-binding mappingをunsupportedとして固定する。後続adapter-selection rowにはpublication-scope identity/version、rule-version change、digest、producer/source-aware-validator、ordinary-reader readinessが残るため、この先行対応だけで同rowをcoveredとはしない。

> **Implementation note (`R8-CORE-028` adapter-defined header-schema closure checkpoint):** `adapter-defined-header-schema-valid` はpublication-mapped Avro schemaとcompatible Kafka header mappingのsupported outcomeにschema adapter provenanceを固定し、schema mapping missing / incomplete / duplicateとheader mapping missing / incompatible / duplicateをそれぞれheader-representation unsupportedとして固定する。`adapter-defined-header-schema-projection-invalid` は完全なAvro schema mappingに対してexposureを定義しないheader ruleをsupportedとする単一projection mismatchである。REDでは現行evaluatorがschema mappingを無視してvalid scenarioを `DM-ADAPTER-003` で拒否し、duplicate mappingの先頭をschema / header / payload-wire / protocol-bindingでsupportedとすることを確認した。exact-one refinementのREDでは、complete mappingとincomplete mappingが同じtupleで重複した場合にpredicate先行のresolverがcomplete側を誤採用することも確認した。GREENではcomplete exact schema mappingを先に解決するpure helperと、tuple一致候補をpredicateより先に数えるexactly-one共通mapping resolverを追加した。`R8-CORE-028` は `covered` となり、Core manifestは206 cases / 151 invalid / 55 valid、one-invalidity auditは151 / 151となる。影響として既存direct-schema output、公開document grammar、rule catalogは変更せず、duplicate mapping hardeningだけを後続adapter-selection checkpointから先行した。次はcommon / inline failure shape と reader trust boundaryのmatrix mappingへ進む。

> **Approved design (`R8-CORE-030`–`R8-CORE-031` failure-shape / trust-boundary matrix checkpoint):** README §8の記載順に従い、common / inline failure shapeのcollapse / exact-reference boundaryを `R8-CORE-030`、reader instruction-authority / structural-escape boundaryを `R8-CORE-031` とする。`R8-CORE-030` はleading collapsed empty subsection、最初のnon-empty subsection後のheaded `none`、exact whole-cell `common:` / `inline:` reference、embedded token rejectionに限定し、後続clauseのreplacement label matching、prohibited replacement subsection、unique reference resolutionは先取りしない。`R8-CORE-031` は `DM-TRUST-001` / `DM-TRUST-002` に限定し、publication safetyは既存 `R8-CORE-029` / `DM-TRUST-003` に維持する。既存versioned fixtureとliteral checker evidenceが十分であるため、fixture、checker、manifestは変更しない。
...
> **Approved order correction and implementation note (`R8-CORE-036` exact schema-target / publication-mapping checkpoint):** 追加監査で、直接schema adapter経路がpublication-scope mappingを解決せず、共通resolverがduplicate exact tupleをmissing mappingと同じ`unsupported`へ畳み込むため、README §3.6 / §8の「duplicate mappings invalidate the scope and fail generation」と、先行`R8-CORE-027` / `R8-CORE-028`のfixture expectationが矛盾していることを確認した。ユーザー承認により、後続の全adapter-selection rowに残していたduplicate outcomeの是正を先行し、schema、payload-wire、header-encoding、protocol-bindingのすべてでexact tuple候補をclass-specific completeness / compatibilityより先に数える。候補0件またはuniqueだがincomplete / incompatibleなら既存のsmallest `unsupported`、complete uniqueならpublication mappingを解決し、2件以上ならsource IDやtargetを開示しない`duplicate-publication-mapping` generation failureとする。専用versioned schema scenarioは直接登録8 target、AsyncAPI 3.0.0 / 3.1.0の`+json` default、case-mismatched unmapped spelling、complete unique Avro mapping、complete / incomplete混在duplicateを固定し、paired invalid scenarioはduplicateを`unsupported`と誤投影する一件だけを`DM-ADAPTER-001`とする。既存payload-wire、nested schema、header duplicate fixtureもgeneration failureへ訂正し、header mappingのduplicate件数判定はschema support判定より先に行う。影響としてCore manifestは222 cases / 163 invalid / 59 valid、one-invalidity auditは163 / 163となり、rule catalog descriptionを実装済みexactly-one semanticsへ同期する。公開document grammar、direct adapter output、missing / incomplete mappingのlocalized unsupportedは変更しない。後続adapter-selection rowにはpublication-scope identity/version、rule-version change、digest、producer/source-aware-validator、ordinary-reader readinessが残る。

> **Approved plan split (`R8-CORE-037`–`R8-CORE-038` source identity / routing-provenance checkpoints):** README §8 の同一段落に並ぶ source/API identity・missing-version marker と routing-provenance closure を、生成時のsource identity resolutionと読取時のtask-scoped source resolutionという責務境界に沿って二つのcheckpointへ分割する。ユーザー承認により、`R8-CORE-037` はAsyncAPI root `id`、explicit authoritative logical-API-identity input、identity未解決、禁止されたtitle / traversal / output-path derivation、およびidentity/version markerの組合せ・source qualification・knowledge・canonical orderを先に固定する。`R8-CORE-038` はrouting / selectionだけを提供するsourceをselected channel fileのrouting-provenance closureで解決する専用fixtureとして次checkpointに残す。影響としてREADMEの記載順、公開document grammar、既存`DM-SRC-003` marker semantics、`facts.core.operationRetrieval` contractは変更せず、coverage ownershipとcommit境界だけを明確化する。

> **Implementation note (`R8-CORE-037` logical API identity / missing-marker checkpoint):** `source-api-identity-resolution-valid` はsource-supplied AsyncAPI root `id`、omitted `id`を補うauthoritative identity input、priority selection、same-priority same-value peers、input不在、ならびに`info.title` / traversal position / output pathが同時に存在してもunknownを維持するcaseをexact factsで固定し、same-priority conflictは`source-api-identity-conflict-invalid`でgeneration failureにする。`source-api-identity-derivation-invalid` はtitle / traversal / output-path derived identityを投影する三caseを単一の集約`DM-SRC-003` mismatchとして拒否する。`sources-api-identity-version-markers-valid` はidentity-only、version-only、両方のunknown row、三source分のmissing-version marker、source-qualified markerのUnicode scalar-value順、root / CONVENTIONSの`knowledge: requires-input`、Schema Evolutionへのverbatim再掲を固定し、source-ID省略とSources marker順序mutationを各単一`DM-SRC-003` concernにする。REDではsource-aware validator未実装を確認し、GREENではsource root `id`→authoritative precedence / conflict→source-qualified unknownの順を解決するpure evaluatorを追加した。完了前監査と独立reviewでSchema Evolution再掲validatorのorder / final placement、whole-section unknownとの併存、根拠のないorphan markerの受理gapを順に再現したため、専用valid / invalid mutationをREDに追加した。shared source-marker predicateをCONVENTIONS state parserとSources validatorで用い、all actual marker-shaped linesがexact expected membership・exact-once・canonical contiguous final groupを満たすことを要求する。影響としてCore manifestは232 cases / 170 invalid / 62 valid、one-invalidity auditは170 / 170となり、公開document grammarとrule catalogは変更しない。次は承認済み分割の`R8-CORE-038` routing-provenance closureへ進む。

> **Approved design (`R8-CORE-038` routing-provenance closure checkpoint):** ユーザー承認により、document-set retrieval factsだけでは「routing / selectionだけに寄与するsource」を機械的に証明できないため、versioned full-profile document setとsource-aware contribution scenarioを組み合わせる。`source-contract`はselected channel本文と通常のoperation-row fact、`source-routing`はselected rowの`Task`だけ、`source-unrelated`は別operationだけに寄与する。contribution recordsからchannelごとのbody / operation-row unionを導出し、selected channelの`source_refs`、exact operation traceのsource IDs、direct Sourcesのloaded index pathを照合する。invalid pairはselected channel metadataからrouting-only sourceだけを削除し、Markdown grammarではなくclosure mismatch一件を`DM-IDX-007`にする。影響としてREADME §3 / §3.2 / §6.1の既存routing-provenance semantics、公開document grammar、rule catalog、`facts.core.operationRetrieval` contract、後続checkpoint順序は変更せず、source-aware fixture evaluatorとcoverage ownershipだけを追加する。

> **Implementation note (`R8-CORE-038` routing-provenance closure checkpoint):** `routing-provenance-closure-valid` は三sourceの寄与を分離し、`channels/orders.md`のclosureを`source-contract, source-routing`、routing-only subsetを`source-routing`、selected exact traceを同じ二sourceと`INDEX.md`に固定し、rootのaggregate `source_refs: all`に含まれる`source-unrelated`をtask-scoped provenanceへ展開しない。paired invalid scenarioはchannel metadataから`source-routing`だけを除き、単一の集約`DM-IDX-007`で拒否する。REDではsource-aware validator不在を確認し、GREENではoperation-row fieldを既存標準列に限定して寄与先channelを実operation rowから解決し、channel本文との和集合、routing-only差分、CONVENTIONS / required-contextを含むselected source setを導出するpure evaluatorとvalidatorを追加した。mutation後metadataの再解析をfixture helperに限定し、versioned valid setは既存projection manifestでrestampした。独立reviewでnon-array `contributions`が例外終了するgapとvalid / invalid寄与記録のdrift riskを再現したため、malformed containerを単一`DM-IDX-007`へ畳み込むno-throw testを追加し、invalid scenarioをvalid scenario参照＋mutationだけにしてmutation以外の同一性を固定した。影響としてCore manifestは234 cases / 171 invalid / 63 valid、one-invalidity auditは171 / 171となり、production document-set validatorと既存retrieval factsは変更しない。次はRemaining Core Inventoryの順序どおりmedia-type canonicalization / replacement identity clauseへ進む。

> **Approved plan split (`R8-CORE-039`–`R8-CORE-040` media-type pipeline checkpoints):** README §8で一続きになっているmedia-type canonicalization、parameterized wire-adapter、post-adapter replacement identityのclauseを、adapter選択前のsource media-type boundaryと、adapter選択後のemitted identity boundaryに分割する。ユーザー承認により、`R8-CORE-039` はRFC 9110 octet grammar、initial canonical target、invalid sourceのgeneration failure、canonical targetによるexact mapping解決を先に固定する。`R8-CORE-040` はselected exact-version adapterがpreserveまたはproven normalizationしたpost-adapter media typeをmedia marker、same-as comparison、uniqueness、payload-representation replacement identityで一貫して使うことと、ASCII / multibyte / embedded `: ` / leading-zero / byte-mismatch / invalid-media-typeのreplacement grammarを次checkpointで固定する。影響としてREADMEのnormative grammarと`R8-CORE-027`の既存adapter evidenceは変更せず、同じpipeline clauseのcoverage ownershipとcommit境界をpre-adapter / post-adapterで明確化する。

> **Implementation note (`R8-CORE-039` pre-adapter media-type canonicalization checkpoint):** `media-type-canonicalization-valid` はtype / subtype / parameter-nameのASCII case folding、parameterのASCII順、token / quoted-string等価、quoted-pair、parameter-value case保持、embedded `: `、空parameter entry、semicolon OWS、U+00FF超のUTF-8、multibyte隣接quoted-pair、NFC / NFD非正規化をcanonical `effectiveTarget` factsとして固定し、invalid `=` whitespace、case-fold後のduplicate parameter、type/subtype separator欠落を`invalid-media-type` generation failureにする。noncanonical source spellingから得たcanonical targetだけがexact publication mappingを解決するcaseも含める。paired invalid scenarioはinvalid syntaxをmissing-adapter unsupportedとして投影する旧挙動を単一`DM-ADAPTER-002` mismatchとして拒否する。REDではvalid scenarioが`DM-ADAPTER-002`となりinvalid pairが診断なしになることを確認し、GREENではpayload-wire adapter分岐の先頭で既存octet canonicalizerを一度だけ適用し、そのcanonical valueをdirect判定、exact mapping、preserve判定、effective targetへ渡した。影響としてCore manifestは236 cases / 172 invalid / 64 valid、one-invalidity auditは172 / 172となる。既存canonical入力のadapter output、公開document grammar、rule catalogは変更せず、非canonical valid sourceは正規化後にadapter選択し、invalid sourceはunsupportedではなくgeneration failureになる。次は承認済み分割の`R8-CORE-040` post-adapter emitted identity / replacement identity checkpointへ進む。
...
> **Approved plan split (`R8-CORE-042` source-shard provenance / token-measurement checkpoints):** README §8のsource-shard clauseを、catalog-cell contributor provenanceとfixed-point resolution、および実token measurement evidenceの二checkpointに分ける。ユーザー承認により、第一checkpointは別shardのauthoritative inputがcatalog cellを供給するsource-aware scenario、transitive contributor chain / cycle、duplicate rowや重複loadのないexact fixed-point factsを固定し、`R8-CORE-042`を`partial`として記録する。第二checkpointはtransitive / false-positive shardを漏れなく含むtokenizer-versioned measurement evidenceを独立して設計し、同rowを`covered`にする。影響としてREADMEのclause順は維持し、runtime source semanticsと測定方法・tokenizer依存を分離する。Task 14のcomplete-surface token evidence interfaceをこのcheckpointで先取りせず、第一checkpointではproduction document grammar、runtime document-set interface、token measurement toolingを変更しない。

> **Implementation note (`R8-CORE-042` source-shard provenance / fixed-point checkpoint):** `source-shard-provenance-fixed-point-valid`は既存task-scoped `sources-sharded-cycle-valid`を読み、別shardのprovider `z`が`a`の`Location=a.json` cellを供給するauthoritative contributionを宣言する。source-aware evaluatorはscenarioのpathを期待値として信頼せず、parsed source rows、shard ownership、`source_refs`、`sourceResolutions`からtarget/provider shard、`a → z → a` contributor edges、requested/resolved IDs、loaded pathsを導出する。paired invalid scenarioはtarget shardの`source_refs`から`z`だけを除き、単一primary `DM-SRC-005`で拒否する。既存fixtureがtask-scoped identity契約であるためwhole-set digestは再検証せず、source resolution責務だけを検証する。`DM-SRC-005` catalog descriptionは既存READMEどおり、source shard自身のrowに加えて任意のrowまたはlocalized markerへfactを提供するsourceのclosureを明記した。独立reviewでnon-array rows / shards、shard row欠落、fixed-point配列欠落が例外終了するgapを再現したため、境界入力を正規化して単一`DM-SRC-005`へ畳み込むno-throw regressionを追加し、同時にreverse contributor edgeだけを除いたacyclic graphを拒否してcycle判定の過受理を防いだ。影響としてCore manifestは248 cases / 181 invalid / 67 valid、one-invalidity auditは181 / 181となる。production document grammar、runtime document-set interface、token measurement toolingは変更せず、token measurement evidenceが次checkpointに残るため`R8-CORE-042`は`partial`である。

> **Approved design (`R8-CORE-042` source-shard token-measurement checkpoint):** ユーザー承認により、同一のclient-visible Sources catalogを持つ測定専用direct / sharded document-set pairをversioned Core evidenceとして追加する。sharded setの代表taskは、requested sourceのexact shard、inclusive range overlapによるfalse-positive shard、別shardのcatalog-cell contributorを解決するtransitive shard、およびcontributor cycleのfixed-point終了を一つの実retrieval traceで固定し、無関係なshardをloadしない。`tiktoken==0.13.0` / `o200k_base`をartifact-localに固定したsource-shard専用builderは、format-specific retrieval instructions、selection / resolution tool-result context、ordered document unitsのexact UTF-8 bytesを明示的なcanonical envelopeへ連結してDocAI shardedとdirect baselineを同じtask boundaryで計測する。証拠はinput digest、各unit digest / byte length、document / instruction / tool-result / total token breakdown、nearest-rank p50 / p95 / maximum、およびsharded totalがdirect totalを下回る限定的claimを保持する。JS corpus testは両setのwhole-set conformance、Sources rowsの同一性、production resolution factsとevidence traceの一致、全transitive / false-positive shardの測定包含、generated JSON / Markdownのbyte-identical再生成を検査する。影響としてTask 14のcomplete-surface task corpus / public evidence interfaceは前倒しせず、このbuilderとpinはCore source-shard clauseだけに限定する。READMEのruntime grammar、既存document-set facts、他catalogのmeasurement ownershipは変更しない。

> **Implementation note (`R8-CORE-042` source-shard token-measurement checkpoint):** `evaluations/source-shards`は同じ26行のcatalogとprojection manifestを持つwhole-set-validなdirect / sharded pairを固定し、`CONVENTIONS.md`の`source_refs: b`からoverlap route `a-c`とexact route `b`を考慮・loadし、`a-c`をfalse positive、catalog-cell contributor `z`をtransitive loadとして`a, b, c, z`へ収束させる。`d-y` shardの22行はclient-visible catalogに残るがtaskではloadしない。REDではevidence input不在、projection manifestのcatalog不在、ならびに短いplaceholder Locationを使った初回実測でshardedがdirectより216 tokens増えることを順に確認し、unfavorable shardingをvalid savings claimにしないtestが機能することを確かめた。測定用catalogの未選択22 sourceをcomplete protocol / binding / schema / message / behavior / security authorityへのlong-form Locationとしてdirectとshardedの双方で一致させた最終fixtureでは、artifact-local `tiktoken==0.13.0` / `o200k_base`でcanonical UTF-8 envelopeを実測し、sharded 1688、direct 1791、p50 / p95 / maximumの全てで103 tokens（5.751%）削減した。builderの`--check`はJSON / Markdownのbyte-identical再生成、`--validate`はtokenizer未導入環境でもinput / projection / loaded-unit digestとtraceのdriftを検査する。claimは`exact source b resolution`のraw input tokensだけに限定し、cache、billed-token、compact、complete-surface savingsを主張しない。影響としてCore manifest / one-invalidity件数は248 / 181のまま、production grammarとruntime factsは変更せず、`R8-CORE-042`を`covered`にする。Task 14は代表task corpusと全catalog / fallbackを扱う独立checkpointとして残る。

> **Approved review hardening (`R8-CORE-042` synthetic conformance evidence):** 独立reviewで、初期projection manifestがemitted catalogのsnapshotだけを含み、authoritative inputのexact bytes、cross-source contribution、precedenceをprojection identityへ十分に結び付けていないこと、token-free `--validate`がrecorded token totalsと一部measurement metadataの改変を検出しないこと、ならびにPython 3.9で`zip(..., strict=True)`が例外になることを確認した。ユーザー承認により、この証跡を実運用性能benchmarkではなくREADME §3.2 / §6.2のsynthetic conformance evidenceとして明記し、(1) versioned authoritative source filesとprojection configurationの全byte digestをmanifestへ固定してvalidatorで再照合する、(2) shardingが有利なpositive caseと不利なnegative controlの双方を同じ測定器で記録し、結果に応じたemission decisionを検証する、(3) token-free validationでcanonical envelope、document bytes、mirrored metadata、集計、claim arithmeticを再構築し、pinned tokenizerの`--check`を別の再現可能な必須gateとして維持する、(4) Python 3.9互換を回復する設計へ変更する。影響として当初の1688 / 1791 tokensはprovisionalとなり、hardening完了まで`R8-CORE-042`は`partial`へ戻す。READMEのnormative grammar、runtime document-set facts、Core manifest / one-invalidity件数、Task 14のcomplete-surface evidence責務は変更しない。

> **Implementation note (`R8-CORE-042` projection-input identity hardening checkpoint):** `source-inputs/authoritative-sources.json`は26 sourceをsynthetic authority domainごとに固定し、source `b`自身には`Location`を置かず、source `z`のauthoritative contributionだけがそのcellを供給する。`source-inputs/projection-config.json`はcatalog row order、contribution-over-self precedence、複数providerとmissing cellのgeneration-failure、direct / sharded representation routesを宣言する。projection manifestは両inputのexact UTF-8 byte length / SHA-256を保持し、builderはdigestを再照合してauthoritative inputから26行catalogを導出し、manifest catalogとmeasurement task contributionの双方へexact comparisonする。両document setは新manifestでrestampし、`--validate`もmanifest digestからshort IDを導出して全rootのshared projection identityを検査する。reviewでrootとrecorded identityを同時改変すると検証をすり抜けるgap、およびtask contributionのauthority不一致を再現したため、それぞれmutation regressionを追加した。`zip(strict=True)`をlength check済みの通常zipへ変更し、Python 3.9.21でのtoken-free validationも固定した。projection IDのtokenization変化によりpositive caseの再生成値はsharded 1672 / direct 1795となるが、positive / negative controlとenvelope / totals改変耐性が次checkpointに残るため数値は引き続きprovisionalで、`R8-CORE-042`は`partial`のままとする。Core manifest / one-invalidity件数、production grammar、runtime facts、Task 14責務は変更しない。

> **Implementation note (`R8-CORE-042` token-free evidence-integrity checkpoint):** tokenizer-backed generationとtoken-free `--validate`が同じ`assemble_run()`を使い、shared context、format-specific instruction、canonical tool result、ordered document bytesからcanonical envelopeを再構築するようにした。`--validate`はtop-level / task / runのexact field setと全mirrored metadata、loaded documentのbytes / digestとnon-negative token metadata、envelope bytes / SHA-256、四componentのtoken sum、boundary deltaとtotal、nearest-rank aggregate、absolute / relative savingsを含むclaim全体を検査する。REDではenvelope digest、document token metadata、component sum、boundary delta、builder metadata、余分なtop-level / run field、malformed task runs、derived savingsを改変し、旧validatorが通過またはMarkdown差分まで遅延することを確認した。GREENでは全改変をtokenizer import前の固有diagnosticで拒否し、malformed containerもtracebackにしない。token-free modeはtokenizerの正しいcountそのものを再計算せず、内部算術とbyte provenanceだけを保証するため、`tiktoken==0.13.0`を使う`--check`が引き続き唯一のtoken-count regeneration gateである。positive / negative synthetic controlsは次checkpointに残るため、`R8-CORE-042`は`partial`を維持する。production grammar、document sets、provisional 1672 / 1795 totals、Core manifest / one-invalidity件数、Task 14責務は変更しない。

> **Approved review adjustment (`R8-CORE-042` load-all disclosure):** 独立reviewでnegative controlが四source shardを実際にloadしていても、既存の`loadAllFallback: false`だけではload-all自体がMarkdown summaryへ明示されず、production resolverの`INDEX.md` fixed-point factsとも直接比較されていないことを確認した。ユーザー承認により、`source_refs: all`が要求する必須load-allを新しい`loadAllSources`、不確実選択等によるfallbackを既存`loadAllFallback`として分離する。negativeのdirect / sharded runは前者をtrue、後者をfalseとし、positiveは双方falseにする。影響としてschema 1.1のtraceとcanonical tool-result contextに一booleanを追加して実測値を再生成し、Markdownはload-allと三fallbackを別々に表示する。document-set bytes、projection identity、runtime grammar、fallback semantics、Task 14責務は変更しない。

> **Implementation note (`R8-CORE-042` positive / negative synthetic-control checkpoint):** evidence schemaとbuilderを`1.1.0`へ更新し、同じsynthetic projection、tokenizer、canonical-envelope測定器、direct / sharded document-set pairに二つのcontrolを固定した。positive control `resolve-source-b-provenance`はexact `source_refs: b`からoverlap false positiveとtransitive contributor cycleを含むfixed pointを測り、sharded 1674 / direct 1797で`emit-source-shards`（123 tokens、6.845%削減）になる。negative control `resolve-all-source-provenance`はroot `source_refs: all`に対応して全26 sourceと四source shardを一度ずつloadし、sharded 2608 / direct 1643で`retain-direct-sources`（965 tokens増加）になる。各taskはcontrol kind、expected / observed emission decisionを保持し、builderとtoken-free `--validate`は同率を含む`sharded >= direct`をdirect維持として再導出する。claimはpositive task IDへ機械的に限定し、全controlに対するunqualified savingsをfalse、negative task regressionを+965 tokens、全task aggregate regressionをp50 +31 / p95 +811 / maximum +811 tokensとして開示する。REDではcontrol pair不在、observed decision改変、load-all disclosure不在を確認し、GREENでは正負の分岐、`loadAllSources`とfallbackの分離、production resolverから得た`INDEX.md` fixed pointとのexact一致、限定claim arithmetic、regression disclosureをliteral assertionsで固定した。`tiktoken==0.13.0`の`--check`は引き続き唯一の実token-count regeneration gateである。これによりreview hardeningの四項目が揃い、`R8-CORE-042`を`covered`へ戻す。production grammar、projection inputs、document-set bytes / identities、Core manifest / one-invalidity件数、Task 14責務は変更しない。

> **Approved plan adjustment (`R8-CORE-043` Reply channel-state checkpoint):** README §8の残存clause順に従い、古いRemaining Core Inventoryに残っていた「additional schema and adapter casesをReplyより先に扱う」という記述を整理し、Reply channel-state境界を次checkpointにする。先行するschema / media-type要件は`R8-CORE-024`–`R8-CORE-028`、`R8-CORE-036`、`R8-CORE-039`、`R8-CORE-040`がすでに所有し、全adapter-classを横断する残件は後続rowが所有する。影響はcoverage rowの作業順とinventory説明に限定し、READMEのnormative clause、既存coverage ownership、production validator、fixture versionは変更しない。

> **Approved design adjustment (`R8-CORE-043` channel-knowledge valid evidence):** 当初設計の二invalid fixtureをREDにした後、既存`reply-states-routing-valid`のwhole-Reply unknownがmessage-selection不足だけを表し、channel知識不足の専用`reply channel requires ...` markerをversioned valid evidenceとして固定していないことを確認した。ユーザー承認により、新しいstandalone valid setへ分散せず、既存Reply state matrixへprimary operation/messageだけを保持するchannel-knowledge fallback operationを追加し、reply message、`reply:` INDEX entry、synthetic operationを生成しないexact factsを固定する。影響として既存valid fixtureのoperation/message期待値を一件拡張するが、Reply grammar、validator semantics、source-aware selection evaluator、既存operationのfactsは変更しない。

> **Implementation note (`R8-CORE-043` Reply channel-state checkpoint):** `reply-states-routing-valid`へ`channel-unknown-reply`を追加し、whole-Reply `unknown`と専用channel-knowledge marker、primary-only Message cellを固定した。`reply-channel-unknown-invalid`はexpanded Replyの`channel: unknown`を単一`DM-REPLY-002`、`reply-whole-fallback-subsection-invalid`はwhole-Reply fallback後の`#### Channel`を単一`DM-REPLY-001`として拒否する。既存のwhole-fallback後key混在、発明されたINDEX `reply:` entry、static / dynamic channel casesと同じexact corpus assertionに統合した。REDでは二caseのmanifest不在とchannel-knowledge operation不在を別々に確認し、GREENではproduction validator変更なしで全境界を受理・拒否した。影響としてCore manifestは250 cases / 183 invalid / 67 validとなり、`R8-CORE-043`を`covered`にする。READMEのnormative grammar、rule catalog、source-aware Reply selection semanticsは変更しない。
...
> **Implementation note (`R8-CORE-044` source-aware selection-rule outcome checkpoint):** `message-selection-source-outcomes-valid`はmulti-message primary / replyそれぞれについて、authoritative selection rule欠落とknown-but-unrepresentable ruleの四状態をsource factsから導出する。primary failureはoperation / Message / INDEX entryを生成せず、欠落を`unknown` Unprojected Operations + `knowledge: requires-input`、表現不能をlocalized `unsupported` + `coverage: requires-source`へ投影する。reply failureはprimary operationとprimary-only INDEX Messageを保持し、reply Message / `reply:` entryを生成せず、欠落をexact `reply message selection rules require ...` whole-Reply unknown、表現不能をwhole-Reply replacement unsupportedへ投影する。paired invalid scenariosはprimary誤投影を単一`DM-IDX-008`、reply誤投影を単一`DM-REPLY-001`で拒否し、diagnosticはsource identity / locationを開示しない。REDではfocused commandが新規三case不在を列挙して1件失敗し、GREENでは同test 1 / 1、one-invalidity audit 189 / 189が成功した。影響としてCore manifestは258 cases / 189 invalid / 69 validとなり、production `core.mjs`へpure evaluatorとstructural validatorを追加して`R8-CORE-044`を`covered`にした。README normative semantics、public document grammar、rule catalog、既存selection evaluatorは変更せず、残るCore inventoryは後続`R8-CORE-*` rowsに限定される。

> **Review hardening (`R8-CORE-044` selection-rule cardinality, fix round 1):** final reviewで、初期evaluatorが`operation.primaryMessages` / `operation.replyMessages`の件数を参照せず、単一Messageでもmulti-message専用fallbackを導出するgapを確認した。既存二invalid fixtureを、対象Message setだけを二件から一件へ減らして旧fallback projectionを保持するcausal mutationへ再利用した。REDではfocused commandが両fixtureを`actual=valid`として報告し、corpus 256 / 258 pass、test 0 / 1となった。GREENではtargetごとのsource Message setをlexical sortして件数を評価し、一件ならprimaryはnormal operation / primary INDEX routing、replyはprimary operationを保持したexpanded Reply / `reply:` routingを導出し、二件以上だけをselection-rule fallbackへ送る。`primary` / `reply`以外のtargetは`generation-failure: invalid-selection-target`へ明示的に分岐する。fixture件数と`R8-CORE-044`の`covered`状態は258 cases / 189 invalid / 69 validのまま維持する。

> **Review hardening (`R8-CORE-044` zero-message cardinality, fix round 2):** 再reviewで、Fix Round 1のevaluatorが一件だけを通常投影へ分岐し、0件をmulti-message selection-rule fallbackへ落とすgapを確認した。既存primary / reply invalid source scenarioをtest内で0件へ変異し、旧missing-selection fallback projectionを保持するcausal mutationを追加した。REDでは両mutationのdiagnosticが空で、期待する`DM-IDX-008` / `DM-REPLY-001`との不一致を同時に確認した。GREENでは対象Message setを0 / 1 / multiに明示分岐し、primary 0件は通常operationを生成しないlocalized unsupported Unprojected outcome、reply 0件はprimary operation / primary INDEX routingだけを保持してreply Message / `reply:` entryを持たないwhole-Reply zero-message replacementへ導出する。publication-safe locationは既存のnormalized `sourceId` / `sourceIdentity`からJSON Pointer escapeを適用して決定し、fixtureの期待値からは読まない。focused mutationとone-invalidity auditの成功後も、fixture件数と`R8-CORE-044`の`covered`状態は258 cases / 189 invalid / 69 validのまま維持する。

> **Review hardening (`R8-CORE-044` independent location / message-set state, fix round 3):** 再reviewで、Fix Round 2がcatalog source IDとUnprojected grouping identityからpublication-safe locationを合成し、さらに欠落・非arrayのMessage setをexplicit emptyと同一視するgapを確認した。既存二invalid source fixtureに独立した`sourceLocation` / `replySourceLocation`と`expectedMessageSetInput`を追加し、identityとlocationが意図的に異なる0件mutation、location欠落、Message set property欠落、非array malformed valueをtest内で実行する。REDでは8境界すべてが旧derived-location / known-zero outcomeとなる差分を同時に確認した。GREENではlocationを非空文字列として検証してverbatim使用し、欠落時はidentity/locationを含まない`generation-failure: publication-safe-source-location-unavailable`へ解決する。またMessage set factを`missing` / `invalid` / `known`に分離し、missing primaryはunknown Unprojected + `knowledge: requires-input`、missing replyはprimaryを保持するexact whole-Reply message-set unknown、invalid normalized setは非開示`generation-failure: invalid-message-set`、known setだけを0 / 1 / multi cardinalityへ送る。focused mutationとone-invalidity auditの成功後に`R8-CORE-044`を`covered`のまま確定し、fixture件数は258 cases / 189 invalid / 69 validから変更しない。

> **Review hardening (`R8-CORE-044` primary-state precedence, fix round 4):** 再reviewで、Reply targetがtarget-specificな`replyMessages`だけを状態検証し、`primaryMessages`欠落・非arrayを空配列として消費してexpanded Replyとreply-only INDEX entryを生成できるgapを確認した。既存reply invalid source scenarioをtest内でprimary set欠落 / 非arrayへ変異し、誤ったcomplete Reply projectionを期待値として与えるreal-code mutationを追加した。REDでは両mutationがdiagnosticなしで受理され、`emit-expanded-reply`と`reply:accepted-reply`を生成した。GREENではReply state / cardinalityより前にprimary set stateを評価し、欠落は既存primary unknown Unprojected outcome、非arrayは既存の最小`generation-failure: invalid-message-set`へ解決する。focused test成功後に`R8-CORE-044`を`covered`へ戻し、fixture件数は258 cases / 189 invalid / 69 validから変更しない。
...
> **Approved design and implementation note (`R8-CORE-046` canonical deviation checkpoint):** README §8の次clauseについて、既存`canonical-deviations-deprecation-valid`が持つ全許可位置のexpanded / none状態、sorted multi-marker、禁止Message heading / Related / failure-shape evidenceを再実装せず、新規`operation-deviation-incomplete-states-valid`でunknown / unsupported状態の直前に保持される独立既知deviationを固定した。REDでは新規valid一件とexample / table / post-table sequence split三件がmanifestに存在しないことを確認した。初回GREENではpost-table splitがvalidとして受理され、Payload field tableを割るmutationが`DM-MSG-001` / `DM-MSG-004` / `DM-MSG-005`の三concernを同時発生させたため、checkpointの意図を維持したままtable splitをChannel Bindings tableへ移して単一`DM-OP-004`にした。production Payload validatorは先頭の非空・Unicode順deviation群を検証し、whole-payload core state以後のdeviationを単一`DM-MSG-004`として拒否する。影響としてCore manifestは262 cases / 192 invalid / 70 valid、one-invalidity auditは192 / 192となり、`R8-CORE-046`を`covered`にする。公開README grammarとrule catalogは変更せず、残るCore inventoryはFailure Handling states、conventions、all-class adapter selectionである。検証結果はdeviation-focused testとone-invalidity audit 2 / 2成功、全test 636中635成功 / 1 skip（既存環境skip）/ 0失敗。

> **Approved design and implementation note (`R8-CORE-047` Failure Handling core-state coverage checkpoint):** README §8の次clauseを、既存`failure-actions-and-shapes-valid`、`failure-deviation-order-invalid`、`failure-state-mixed-invalid`、`DM-FAIL-001`、および`executes the Task 9 DM-FAIL-001 DM-FAIL-002 DM-FAIL-003 DM-CONV-004 states actions and shapes corpus`へ対応付ける。既存valid setとliteral assertionsは、deviationなし / ありのexpanded・`none`・`unknown`・replacement全状態、sorted multiple deviations後のexpanded table、suppression-only deviation-plus-`none`、exact operation membershipを既に固定し、invalid pairはmixed stateと逆順deviationを各単一primary concernで拒否する。common / inline failure shapesは`R8-CORE-041`の所有を維持する。coverage文言だけを検査するREDは実動作を検証しないため、ユーザー承認の文書-only TDD例外としてfixture、validator、manifestを再実装せずmatrixとinventoryだけを更新した。影響としてCore corpusは262 cases / 192 invalid / 70 validのまま、公開grammar、production semantics、rule catalogを変更せず`R8-CORE-047`を`covered`にし、残るCore inventoryはconventionsとall-class adapter selectionである。検証結果はFailure-focused test 1 / 1成功、全test 636中635成功 / 1 skip（既存環境skip）/ 0失敗、`git diff --check`成功。
...
- [x] **Step: Core coverage matrix を完成させる**
  ...
  - [x] `R8-CORE-036`（exact schema targets / source defaults / unique publication mapping / duplicate-scope failure）をversioned source scenarioと全adapter-class resolver semanticsで対応付ける。
  ...
  - [x] `R8-CORE-042`（Source Shardsのcross-shard catalog-cell provenance、fixed-point、transitive / false-positive token measurement）をversioned evidenceで対応付ける。
...
- [x] **Step 5: stop rule を適用する**
  - normative meaning、fixture expectation、checker rule のいずれかがレビュー中に変わった場合は公開を止め、仕様バージョンと fixture version を再評価する。

> **Approved design and implementation note (Task 10 stop-rule and Compatibility Core promotion checkpoint):** Step 4で`PUBLICATION.json`とprojection manifestのbindingを検証するchecker ruleを追加したため公開を一旦停止し、仕様版とfixture版への影響を再評価した。normative meaning、264-case corpusのfixture expectation、`valid/full/` bytes、projection-manifest bytesは不変であり、変更は未公開のdistribution metadataを検査するrelease gateに限定されるため、DocAI Messaging versionとCore fixture versionはいずれも`0.17.1`据え置きとする。root READMEはspecification statusをDraftのまま、publication labelだけをCompatibility Core implementation targetへ昇格し、互換性保証を§3.1.1 / §8のCore規範、`fixtures/core/v0.17.1/`、trusted publication scope、read-only checkerに限定した。complete generator surfaceとCore外構造はimplementation-ready、compatibility-stable、stableのいずれにも昇格しない。文書だけの公開判断であるため文字列固定testは追加せず、既存の全test、Core checker、projection-manifest SHA-256、`git diff --check`をpromotion gateとして再実行した。実測は全652 tests中651成功 / 1 skip（Python 3.9未導入の既存環境skip）/ 0失敗、Core checkerは264 cases / 193 invalid、one-invalidity 193 / 193、未使用rule 0、coverage gap 0、projection-manifest SHA-256一致、`git diff --check`成功。これによりstopを解除し、Task 10のexit criteriaを満たした。
...
- [x] full/compact path、routing form、projection identity、coverage、knowledge、source_refs parity を検証する。
- [x] expanded comparison view を実装し、profile link、identity、`x-`、example canonicalization、`field_defaults`、`same_as` 以外の差を拒否する。
- [x] `field_defaults` の logical column reconstruction、order、duplicate、unknown/inapplicable column、`Meaning=none` を検証する。
- [x] `same_as` の backward same-file target、paired full canonical equality、retrieval-unit discoverability、incomplete target rejection を検証する。
- [x] invalid compact example fence/info string、prose mismatch、standard heading/marker/table/example/order mismatch を検証する。
- [ ] Run: `node --test docai-messaging/tools/tests/*.test.mjs`
- [ ] Expected: 全 test PASS。
...
> **Approved design and implementation note (Task 11 workflow file grammar checkpoint):** 次のcommit-sized checkpointとして、routing済みWorkflow fileから決定的に検証できる本文grammarを`validators/complete-workflows.mjs`へ分離し、`validators/complete.mjs`から合成した。`DM-WF-004`はopening metadata後のwrapper禁止、非空title、1 source line / 1–2 literal sentence introduction、固定4 headingの順序を所有する。`DM-WF-005`はwhole-workflow / section-local deviationの非空・Unicode scalar順・canonical leading placementと、各sectionの排他的な`none` / whole-section `unknown` / exact-unit replacement `unsupported` / expanded stateを所有する。`DM-WF-006`はexpanded Preconditions / Failure and Recoveryのtop-level `- ` source-line list、Stepsのtop-level numbered list、非空`From | Trigger | To` table、記載された`SEND` / `RECEIVE` operation headingのCore definitionとの完全一致を所有する。正規化結果は`facts.complete.workflowDefinitions`へtitle、introduction、deviations、section states / items / steps / rowsとして公開し、Workflow routingに一次診断がある場合は本文検証を実行せずcascadeを避ける。TDDではfacts欠落、invalid title / introduction / fixed sectionsの見逃し、incomplete stateのexpanded誤分類、invalid deviation / core-stateの見逃し、invalid expanded structure / operation referenceの見逃し、rule catalog未登録、自己レビューで見つけたtitle前wrapper proseの見逃しをそれぞれREDで確認してからGREENにした。focusedはrouting + body 15 / 15、全667 tests中666成功 / 1 skip（Python 3.9未導入の既存環境skip）/ 0失敗、Core checkerは264 cases / 193 invalid、one-invalidity 193 / 193、未使用Core rule 0、coverage gap 0。影響はcomplete-scopeのWorkflow本文構造とfactsに限定し、Core validator / corpus / publication gate、既存Workflow routing factsを変更しない。authoritative inputに対するpurpose / successful outcomeの成立、全carried value / failure / timeout branch、全relevant transitionの意味的網羅性は投影文書だけから推測せず、Task 12のsource-aware fixture checkpointに残す。Reference Material、selective conventions、full/compact parityとexpanded equivalence、`field_defaults`、`same_as`、complete checker CLIも後続Task 11 checkpointに残し、上のTask 11項目はまだ完了扱いにしない。

> **Approved design and implementation note (Task 11 Reference Material grammar checkpoint):** 次のcommit-sized checkpointとして、complete-scopeのReference Material grammarを`validators/complete-references.mjs`へ分離し、`validators/complete.mjs`から合成した。`DM-REF-001`は固定title、exact `instruction_authority: none` marker、Content heading、単一content fence以外のwrapper / prose禁止を所有する。`DM-REF-002`は単一fence、empty / `markdown` / `text` info、最小4かつembedded backtick runより1長いexact delimiter、final LF、content内のBOM / CR禁止、fence内structural textのdata扱いを所有する。`DM-REF-003`は全`references/*.md`が少なくとも1 operationのSupplemental contextに所有されることを要求し、Core routingが一次診断を持つ場合はownership診断を保留してcascadeを避ける。正規化結果は`facts.complete.referenceMaterials`へpath、info、delimiter length、final LFを含むexact content、consumer operation namesとして公開する。実装中、共通document-set loaderがfence内のidentity-like dataもtrailer候補に数える既存境界を発見したため、承認を得て全Markdown documentのidentity探索をfence-awareに変更し、閉じたfence内だけをdataとして除外する一方、fence外のduplicate / intermediate trailerは引き続き`DM-ID-001`で拒否する計画変更を加えた。TDDではReference Material facts欠落、invalid fixed structure、invalid fence / normalization、fence内identity-like dataの誤検出、未参照fileの見逃し、invalid Core routingからのownership cascade、rule catalog未登録を順にREDで確認してからGREENにした。focusedは9 / 9、全676 tests中675成功 / 1 skip（Python 3.9未導入の既存環境skip）/ 0失敗、Core checkerは264 cases / 193 invalid、one-invalidity 193 / 193、未使用Core rule 0、coverage gap 0。影響はcomplete-scope Reference Material grammar / factsと、共通loaderのMarkdown fence内identity境界修正に限定し、Core corpus bytes / expectation / publication gate、既存Workflow factsは変更しない。source byteのBOM除去、CRLF / lone CR変換、non-UTF-8 source拒否、trailing blank preservationとprojection identityへの反映はmaterialized outputだけから検証せず、Task 12のsource-aware fixture checkpointに残す。selective conventions、full/compact parityとexpanded equivalence、`field_defaults`、`same_as`、complete checker CLIも後続Task 11 checkpointに残し、上のTask 11項目はまだ完了扱いにしない。

> **Approved design and implementation note (Task 11 selective conventions retrieval checkpoint):** 次のcommit-sized checkpointとして、complete-scopeのselective convention retrievalを`validators/complete-conventions.mjs`へ分離し、operationごとのselector、trusted / untrusted時のselection、required workflow paths、supplemental workflowのwhole-CONVENTIONS fallback、direct workflowのwhole-CONVENTIONS retrievalを`facts.complete.conventionRetrieval`として公開した。省略または`all`はwhole file、trusted `none`はmetadata / identityのみ、信頼できないselectorはwhole fileとし、Reference Materialにはworkflow fallbackを適用しない。required workflowとoperationの機械的に証明可能な`Data Representation`依存は既存`DM-CONV-003`を再利用し、operationがcommon failure shapeを参照する場合の`Error Handling`選択だけを新規complete-scope rule `DM-CONV-005`で要求する。Core routing / conventionまたはWorkflow routing / bodyに一次診断がある場合はretrieval factsを`null`にし、不完全なselectorからの例外とcascade診断を防ぐ。TDDではfacts欠落、required / supplemental / direct workflow retrieval欠落、common failure shapeのclosure欠落、invalid selectorでの例外、catalog未登録を順にREDで確認し、GREEN後にtrusted `none`とReference Material除外の境界を追加した。complete rule追加によりCore専用correspondence auditがスコープ外ruleをunusedと誤判定する回帰もREDで検出し、catalog全体ではなく`scope: core`だけを監査するよう修正した。focused complete / Core regressionは503 / 503、全682 tests中681成功 / 1 skip（Python 3.9未導入の既存環境skip）/ 0失敗、Core checkerは264 cases / 193 invalid、one-invalidity 193 / 193、未使用Core rule 0、coverage gap 0。影響はcomplete-scopeのselective-convention validation / retrieval factsとCore auditのscope分離に限定し、Core validator / corpus / publication gateと既存Workflow / Reference Material factsは変更しない。Workflow proseからsemantic dependencyを推測せず、source-awareのcomplete closureとwhole-file fallbackを含むtoken evidenceはTask 12 / 13に残す。full/compact parityとexpanded equivalence、`field_defaults`、`same_as`、complete checker CLIも後続Task 11 checkpointに残し、上のTask 11項目はまだ完了扱いにしない。

> **Approved design and implementation note (Task 11 full/compact pair parity checkpoint):** 次のcommit-sized checkpointとして、`validators/complete-profiles.mjs`にfull / compactをそれぞれ既存complete validatorで独立検証した後だけpairを検証する`validateCompleteProfilePair()`を追加した。`DM-PROFILE-001`はexact `full` / `compact` profile、相互に実rootを解決するprofile link、document path集合、DocAI Messaging version、perspective、`projection_id` / `projection_digest`、対応fileの`coverage` / `knowledge` / `source_refs`一致を所有し、profile固有の`set_id` / `set_digest`は一致を要求しない。`DM-PROFILE-002`はSources / Operations / Workflows / Unprojected Operationsそれぞれのdirect-or-sharded formとshard paths、および正規化済みWorkflow `Name` / `Details`を所有する。片側の独立検証が失敗した場合はpair factsを`null`にし、`DM-PROFILE-*`のcascadeを出さない。TDDではvalidator不在、profile-linkの別root解決、profile / version / perspective / projection identity drift、path不一致、対応file metadata drift、Operations catalog parity、rule catalog未登録を順にREDで確認してGREENにし、その後Sources / Workflows / Unprojected Operationsと同一direct form内のWorkflow routing-name driftを境界testとして追加した。自己レビューでwhole-set既定経路を既存Core focused pairに適用すると`DM-ID-003`になることを検出したが、このfixtureは既存manifest / validatorどおりtask-scoped identityであるため、productionの既定whole-set検証は維持し、unit testだけを`wholeSet:false`に戻した。実測はfocused complete / Core regression 547 / 547、全702 tests中701成功 / 1 skip（Python 3.9未導入の既存環境skip）/ 0失敗、Core checkerは264 cases / 193 invalid、one-invalidity 193 / 193、未使用Core rule 0、coverage gap 0。影響はcomplete-scope pair validation / factsとrule catalogに限定し、Core pair validator / corpus / publication gate、single-set complete validationは変更しない。expanded comparison view、adapter-specific example canonicalization、`field_defaults`、`same_as`、complete checker CLIは後続Task 11 checkpointに残す。

> **Approved design and implementation note (Task 11 expanded comparison checkpoint):** 次のcommit-sized checkpointとして、`validators/complete-profile-comparison.mjs`に対応fileごとの順序付きexpanded comparison viewを追加し、前段の独立complete validation、`DM-PROFILE-001` identity/path parity、`DM-PROFILE-002` catalog parityがすべて成功した後だけ`DM-PROFILE-003`で比較する。viewはopening metadataの標準6 keyを保持して`profile`だけを`full`へ正規化し、root INDEXのprofile-link、profile固有identity trailer、`x-` metadata / one-line marker / self-bounding heading block / table suffix columnsを除外する。標準tableはescape decode済みlogical cells、proseはLFと行末ASCII space、JSON exampleは既存exact parserによるobject-order非依存・任意精度decimal valueで比較し、それ以外のfence data、heading、marker、fixed value、logical table cell、prose value、順序、追加、欠落はexact contractとして保持する。不一致診断は最初の対応compact fileへ結び付け、前段pair診断とのcascadeを出さない。TDDではprose差の見逃し、`x-`差の誤拒否、JSON object order / `1`系number spellingの誤拒否とIEEE 754境界値差、固定値の行末space過受理を順にREDで確認してGREENにした。境界testでmarker / table cell / prose addition・order / Workflow display heading差、table source spacing、Reference Material fence contentを固定し、自己レビューで非JSON fenceの行末spaceをprose扱いする過受理と非rootのprofile-link様文字列を除外する過受理をそれぞれREDで再現して除外境界を修正した。実測はfocused profile pair 37 / 37、全719 tests中718成功 / 1 skip（Python 3.9未導入の既存環境skip）/ 0失敗、Core checkerは264 cases / 193 invalid、one-invalidity 193 / 193、未使用Core rule 0、coverage gap 0。影響はcomplete-scope full/compact pair comparisonとrule catalogに限定し、Core validator / corpus / publication gate、single-set complete validation、既存pair identity / catalog factsは変更しない。`field_defaults` logical reconstruction、`same_as` target expansion / paired-full canonical equality、invalid compact example fence / info、complete checker CLIは後続Task 11 checkpointに残す。

> **Approved design and implementation note (Task 11 field-defaults checkpoint):** 次のcommit-sized checkpointとして、compact限定の`**field_defaults**:`をline-preservingなdocument-set clone上で展開する`validators/complete-field-defaults.mjs`を追加し、その同じ展開処理をsingle-set complete validationとfull/compact expanded comparisonで共有した。新規complete-scope rule `DM-PROFILE-004`はHeaders / Payload field tableへの直前配置、SEND / RECEIVEのdirection-correct logical columns、`Required=yes|no` / `Presence=always|optional` / `Nullable=yes|no` / `Meaning=none`、logical left-to-right order、duplicate、unknown / inapplicable column、default済みcolumnのcompact table残存、full-profile使用禁止を所有し、trailing `x-` columnsを保持する。`Meaning=none`はliteral `none`ではなく空cellへ復元し、paired fullとの既存`DM-PROFILE-003`比較がuniformなfull-side cell equalityを確認する。展開cloneはmarkerを空行へ置換しtable行数を維持する一方、元bytesを保持するためset / projection identity検証の入力は変更しない。TDDではSEND payloadが既存Coreの`DM-MSG-001/004`になるREDとrule未登録のREDを確認してからGREENにし、RECEIVEの`Meaning=none`、Headers、trailing `x-`、facts、duplicate / order / unknown / inapplicable / prohibited value / retained column / full marker / misplaced markerを追加した。自己レビューでReply配下の`##### Headers` / `##### Payload`を当初拒否する境界をREDで再現し、primaryの`####`とnested Replyの`#####`をともに許可した。実測はfocused profile pair 50 / 50、全732 tests中731成功 / 1 skip（Python 3.9未導入の既存環境skip）/ 0失敗。§3.4どおりtoken-savingsは通常validatorが証明せずproducer assertionとして維持し、§6.2 evidenceを伴う検証はTask 12 / 13に残す。影響はcomplete-scope field-default grammar / facts / pre-expansionと`DM-PROFILE-004` catalogに限定し、Core validator / corpus / publication gate、既存profile identity / routing / comparison ruleの意味は変更しない。`same_as` target expansion / paired-full canonical equality、invalid compact example fence / info、complete checker CLIは後続Task 11 checkpointに残す。

> **Approved design and implementation note (Task 11 same-as checkpoint):** 次のcommit-sized checkpointとして、compact channel file内の`**same_as**:`をCore検証前に展開する`validators/complete-same-as.mjs`を追加し、その同じ展開処理をsingle-set complete validationとfull/compact expanded comparisonで共有した。新規complete-scope rule `DM-PROFILE-005`はexact primary / Reply Message target grammar、同一file内のdirect backward expanded target、Required / Presence direction semantics、`x-retrieval-unit: channel-file`、完全なrepresentation境界、full-profile / raw binary / unknown field collection / collection-level unknown / unsupported / recursive / failure-shape禁止を所有する。paired validatorはcompact target / reference factsから対応する二つのfull representationを解決し、既存のtable cell normalization、任意精度JSON equality、prose normalizationを再利用してcanonical equalityを検証する。`field_defaults`は先に展開し、`same_as` factsと診断をcomplete resultへ公開する一方、元document bytesとidentity入力は変更しない。TDDでは有効なbackward referenceのCore拒否とrule未登録から開始し、paired-full mismatch、retrieval metadata欠落、full使用、raw target、malformed / forward / non-local target、placement / adjacent content、SEND / RECEIVEおよびReply direction、Reply target、両field-collection unknown、recursive target、failure shapeを順にRED→GREENにした。direction testの調査ではMarkdown scannerのheading textが`## `を除去することを確認し、scannerの実際の値に合わせて判定を修正した。実測はfocused profile pair 72 / 72、全754 tests中753成功 / 1 skip（Python 3.9未導入の既存環境skip）/ 0失敗、Core checkerは264 cases / 193 invalid、one-invalidity 193 / 193、未使用Core rule 0、coverage gap 0。§3.4どおりtoken-savingsはproducer assertionとして維持し、§6.2 evidenceを伴う検証はTask 12 / 13に残す。影響はcomplete-scope same-as grammar / facts / pre-expansion、paired-full canonical comparison、`DM-PROFILE-005` catalogに限定し、Core validator / corpus / publication gate、既存profile identity / routing ruleの意味は変更しない。invalid compact example fence / infoとcomplete checker CLIは後続Task 11 checkpointに残す。

> **Implementation note (Task 11 compact-example equivalence coverage checkpoint):** README §3.4 / §8のfull/compact equivalence境界を閉じ、fullのpretty JSONとcompactのone-line JSONでobject orderおよびexact decimal spellingだけが異なる有効pairを明示した。invalid compact fence info stringは構成setの`DM-MSG-004`、trailing commaとduplicate object nameを含むinvalid JSON renderingは`DM-MSG-005`がpair comparison前に拒否し、validだがcanonical decoded valueが異なる例だけを`DM-PROFILE-003`が拒否するrule ownershipを固定した。prose、heading、marker / fixed value、logical table cell、canonical example value、ordering、additionのmismatchは既存focused casesで継続して網羅する。追加したcharacterization casesは既存Core validatorで最初から成功したためproduction codeは変更せず、focused全体の初回REDは新しいtop-level test名がprofile rule correspondenceを欠いたことによるものだった。既存audit契約に沿ってpair側の責務を`DM-PROFILE-003`へ明示してGREENにし、focused profile pairは76 / 76となった。影響はcomplete profile-pair fixture coverageとTODO記録に限定し、Core / complete validator、rule catalog、corpus、publication gateの意味は変更しない。次はTask 11のcomplete checker CLI checkpointへ進む。

> **Approved design and implementation note (Task 11 read-only complete checker CLI checkpoint):** `check-complete-fixtures.mjs [candidate-root]`を追加し、candidate root直下の`full/`と`compact/`をwhole-set modeでloadして既存`validateCompleteProfilePair()`へ渡すread-only entry pointを固定した。引数省略時はTask 12で作成予定の`fixtures/complete-candidates/v0.17.1/`を選び、現時点のself-testは既存profile pairを一時領域へcopyし、同一のexplicit projection manifestで両rootをrestampしてwhole-set identityを成立させた明示candidate pathを使用する。成功時はpath数をstdoutへ出し、不正な引数数、profile root欠落、構成set / pair diagnosticsはstderrとexit 1で拒否する。TDDのREDではCLI未実装による4件の`MODULE_NOT_FOUND`を確認し、GREENではvalid pairの実行前後bytes不変、compact prose mismatchを同じmanifestで再restampしても`DM-PROFILE-003`となること、argument count、missing profile rootsを4 / 4で固定した。checkerはrestamp、repair、source-aware inferenceを行わない。実測は全762 tests中761成功 / 1 skip（Python 3.9未導入の既存環境skip）/ 0失敗、Core checkerは264 cases / 193 invalid、one-invalidity 193 / 193、未使用Core rule 0、coverage gap 0。Task 12 / 13で追加するcandidate `cases.json`、complete `COVERAGE.md`、source traceability、token evidenceのgateはまだ扱わず、既定pathでのno-argument成功もcandidate作成後に検証する。
...
- [x] Core source scenario を拡張し、required/supplemental workflows、Reference Material、multiple source/index shards を追加する。
- [x] full set に flat と sharded catalog の代表を含め、各 shard の false-positive/fallback retrieval task を定義する。
- [x] workflow の全 section を expanded/none/unknown/unsupported で表す separate cases を作る。
- [x] Reference Material の instruction authority、fence length、UTF-8 normalization、forbidden target cases を作る。
- [x] tagged/untagged polymorphism、raw binary、adapter-defined structured non-JSON representation を作る。
- [x] compact set を full と同じ paths で作り、compact example、field defaults、same-as、selective conventions を実際に使う。
- [x] complete candidate の同じ explicit projection-input manifest を `--projection-manifest <candidate-manifest-path>` で指定し、`restamp-document-set.mjs --write` で full/compact root を個別に restamp する。manifest は auto-discover せず、同じ manifest での dry-run 再実行が両方 `restamp required: no` になることを確認する。
...
> **Implementation note (Task 12 required/supplemental context candidate checkpoint):** 次のcommit-sized checkpointとして、candidate-local `source/complete-contexts.json`にalpha required Workflow、alpha supplemental Workflow、middle supplemental Reference Materialのauthoritative factsを追加し、exact source bytesをcandidateのexplicit projection-input manifestへSHA-256 bindingした。full/compactの同じ3 pathsに、cross-operation `a-operation`→`m-operation` sequencingとtimeout recoveryを持つrequired Workflow、synthetic trace用supplemental Workflow、`instruction_authority: none`と4-backtick Markdown fenceを持つReference Materialを配置した。operation shard routingはalphaのrequired / supplemental contextとmiddleのsupplemental referenceだけを選択し、zetaはcontext-freeのまま維持してunrelated advanced structureからreadiness / retrievalが独立する境界を固定した。TDDのREDではsource scenario不在と両profileのcontext cell欠落を3 / 3で確認し、GREENではmanifest binding、exact routing、Workflow title / steps / transitions、Reference content / ownership、zetaの空required/supplemental retrievalを3 / 3で確認した。実測は全766 tests中765成功 / 1 skip（Python 3.9未導入の既存環境skip）/ 0失敗、complete checkerは10 pathsのfull/compact canonical equivalence、Core checkerは264 cases / 193 invalid、one-invalidity 193 / 193、未使用Core rule 0、coverage gap 0、両profileのexplicit-manifest dry-runは`restamp required: no`となった。このcheckpointではdirect Workflowsだけを扱い、multiple source / operation / Workflow shards、全Workflow state、source-to-projection semantic trace、compact固有表現は後続Task 12 checkpointに残すため、上のTask 12第1項目を含むcheckboxはまだ完了扱いにしない。

> **Implementation note (Task 12 sharded catalog retrieval checkpoint):** candidateのdirect Sourcesを、`complete-contexts` / `storefront-behavior`を持つbroad shardと、そのinclusive range内の`storefront-asyncapi-3.1.0`を持つnarrow shardへ置き換え、full/compactの同じ2 source-index pathsを追加した。これによりSourcesとOperationsはoverlapping sharded catalog、Workflowsはdirect flat catalogとして同じcomplete candidate内に共存する。TDDのREDでは両profileがまだdirect Sourcesであることを2 / 2で確認し、GREENではnarrow source exact selectionが両source shardsを読みbroad shardのrowsをfalse positiveとして解決すること、broad endpoint selectionがbroad shardだけを読むこと、root `source_refs: all`が両source shardsをload-allすること、既存`m-operation` exact selectionがbroad operation shardをfalse positiveとして読むこと、operation semantic fallbackが両operation shardsを読むことを5 / 5のcandidate integration testでliteralに固定した。実測は全768 tests中767成功 / 1 skip（Python 3.9未導入の既存環境skip）/ 0失敗、complete checkerは12 pathsのfull/compact canonical equivalence、Core checkerは264 cases / 193 invalid、one-invalidity 193 / 193、未使用Core rule 0、coverage gap 0、両profileのexplicit-manifest dry-runは`restamp required: no`となった。source / operation shard token measurement、Workflow Shards、source-to-projection semantic traceは後続Task 12–14に残す。

> **Implementation note (Task 12 Workflow section-state candidate checkpoint):** 既存の`alpha-delivery` / `alpha-observability`を全4 sectionのexpanded casesとして維持し、full/compactの同一pathsへ、全sectionがそれぞれ`none`、whole-section `unknown`、replacement `unsupported`となる3 Workflowを追加した。authoritative `complete-contexts` sourceは各stateと不足入力または表現不能featureを保持し、新規Workflow群を`a-operation`のsupplemental contextおよび関連operationの`Related` navigationから到達可能にした。unknown fileは`knowledge: requires-input`、unsupported fileは`coverage: requires-source`、rootは両方を集約し、context-freeな`z-operation`のrequired / supplemental retrievalは空のまま維持する。TDDのREDでは新規Workflowとsupplemental routingの欠落を両profileで4 / 4失敗として確認し、GREENでは4 section × 4 statesのliteral matrix、routing、root/file completenessを4 / 4で固定した。source bytesをcandidateのexplicit projection-input manifestへ再bindingして両profileを個別にrestampした。実測は全770 tests中769成功 / 1 skip（Python 3.9未導入の既存環境skip）/ 0失敗、complete checkerは15 pathsのfull/compact canonical equivalence、Core checkerは264 cases / 193 invalid、one-invalidity 193 / 193、未使用Core rule 0、coverage gap 0、両profileのexplicit-manifest dry-runは`restamp required: no`となった。このcheckpointはdirect Workflowsのvalid state matrixだけを所有し、Workflow Shards、intro / list / table / deviationのinvalid focused cases、source-to-projection semantic trace、token measurementは後続Task 12–14に残す。

> **Implementation note (Task 12 Reference Material normalization checkpoint):** 既存の`middle-operations` Reference Material sourceを、先頭BOM、CRLF、literal 4-backtick run、decomposed Unicode（`e` + U+0301）、末尾空行を含むraw contentへ拡張した。full/compact projectionはBOMを除去してLFへ正規化する一方、decomposed Unicodeと末尾LFをbyte-levelで保持し、内容中の最長runより一つ長い最小5-backtick fenceおよび固定`instruction_authority: none`を使用する。source-aware negative scenariosは実candidateの`m-operation` rowを変異させ、Reference Materialをrequired contextへ置く場合とchannel fileをsupplemental contextへ置く場合がともに`DM-IDX-005`となることを固定した。TDDのREDはsource binding、forbidden target scenarios、両profile projectionの4 / 4失敗を確認した。最初のGREENは3 / 4で、test mutationがfixtureの実Summary `Handles the middle event range`ではなく誤った文言を探索していたことをsystematic debuggingで特定し、production fixtureを変えずtest premiseだけを修正してfocused 4 / 4をGREENにした。source manifestを再bindingし、両profileを同じexplicit manifestでrestampした。最終実測は全771 tests中770成功 / 1 skip（Python 3.9未導入の既存環境skip）/ 0失敗、complete checkerは15 pathsのfull/compact canonical equivalence、Core checkerは264 cases / 193 invalid、one-invalidity 193 / 193、未使用Core rule 0、coverage gap 0、両profileのexplicit-manifest dry-runは`restamp required: no`となった。影響はcandidate source/projectionとsource-aware fixture testsに限定し、Unicode normalizationそのものは行わない。Reference Materialのstructural escapeおよびnon-UTF-8 rejectionはTask 13のfocused invalid corpusに残す。

> **Approved design and implementation note (Task 12 polymorphic and raw representation checkpoint):** `tagged/untagged polymorphism、raw binary、adapter-defined structured non-JSON representation`は、既存validatorが扱うpolymorphism / raw grammarと、trusted publication mappingおよびadapter-specific example validationをcheckerへ渡す設計が必要なnon-JSON grammarに分割した。今回のcheckpointではcandidate-local `source/complete-representations.json`を追加してmanifestへexact bytesをbindingし、full/compactの同一`channels/representations.md`へ、exact discriminator `const`を持つSEND tagged JSON variants、制約されたlabelとreceive-side Presence tableを持つRECEIVE untagged JSON variants、example / field tableを持たず内容・2 MiB上限・SHA-256 integrity ruleをproseで保持するSEND `application/octet-stream` opaque raw representationをmaterializeした。3 operationは既存broad operation shardへ追加し、source shardは新source rowを含むfixed-point resolutionを維持する。TDDのREDはsource bindingと両profile channel欠落の3 / 3失敗、GREENはcandidate integration 11 / 11で確認し、production validatorは変更しなかった。両profileを同じexplicit manifestでrestampし、complete checkerは16 pathsのcanonical equivalenceを確認した。最終実測は全774 tests中773成功 / 1 skip（Python 3.9未導入の既存環境skip）/ 0失敗、Core checkerは264 cases / 193 invalid、one-invalidity 193 / 193、未使用Core rule 0、coverage gap 0、両profileのexplicit-manifest dry-runは`restamp required: no`となった。このcheckpointはstructured contentをrawへ偽装せず、variantにunlabeled example / common tableを置かない境界を固定する。adapter-defined structured non-JSONはpublication-scope adapterを組み込み扱いにしないため次checkpointへ残し、上のTask 12 checkboxは未完了のままとする。

> **Approved design and implementation note (Task 12 adapter-defined structured non-JSON checkpoint):** 前checkpointで分離したnon-JSON grammarを、publication-scope identity/version、exact `payload-wire` target `text/csv;charset=utf-8`、rule version `complete-fixture-csv-1.0.0`の全一致時だけ有効になるtrusted CSV adapterとして実装した。Core document-set validatorは呼出側が明示したadapterだけをprimary / reply Message、variant、inline / common failure-shapeのexample validationへ渡し、未指定、duplicate、tuple / rule mismatchは従来どおり`DM-MSG-004`またはcaller固有ruleでdefault-denyする。CSV ruleはLF境界、unique non-empty header、exactly one data row、列数、quote / doubled-quote grammarを検証し、exact JSONと共通の`Map` decoded-value modelへ変換して既存field path / type / constraint検証を再利用する。fullはquoted、compactはunquotedの同値exampleを使用し、profile comparisonとpaired `same_as` canonical viewはadapter identity / rule versionを含むdecoded valueで比較する。complete checkerはcandidate-local manifest bytesのSHA-256が両rootの`projection_digest`と一致する場合だけmappingを渡し、restampのstaged final validationも明示manifestから同じtrusted ruleを解決する。TDDのREDは明示adapterがあっても`DM-MSG-004`になるvalidator、trusted registry不在、candidate source / projection欠落、checkerのCSV拒否、restamp staged validationの`DM-MSG-004`を順に確認し、GREEN後に手製decoderを実adapterへ置き換えた。自己レビューでfailure-shape callerへのadapter未伝播と、exampleからschemaを推測しかねないsource scenario不足を検出したため、inline failure-shapeの`DM-FAIL-003`をREDにして全callerへoptionを渡し、CSV sourceへ直接登録済みAsyncAPI 3.1 Schema Object targetとrequired / openness / field semanticsを追加するRED→GREENを行った。source scenario、broad operation shard、full/compact `channels/representations.md`、manifest source digest / adapter mappingを更新して両profileを個別restampした。最終実測は全781 tests中780成功 / 1 skip（Python 3.9未導入の既存環境skip）/ 0失敗、Core checkerは264 cases / 193 invalid、one-invalidity 193 / 193、未使用Core rule 0、coverage gap 0、complete checkerは16 pathsのcanonical equivalence、両profileの同一explicit-manifest dry-runは`restamp required: no`となった。影響は明示的なexample-adapter option、trusted complete-fixture registry、checker / restamp adapter入力、candidate CSV representationに限定し、JSON direct adapter、opaque raw grammar、Core corpus / publication gate、ordinary-reader contractは変更しない。次はTask 12の記載順どおり、compact example / `field_defaults` / `same_as` / selective conventionsのcandidateでの実使用を独立checkpointとして進める。

> **Approved design and implementation note (Task 12 compact forms and selective conventions checkpoint):** source-definedで同じschema / exampleを持つ`r-json-original-operation`と`r-json-reuse-operation`を同一channel fileへ追加し、fullは両representationをexpanded、compactは先行representationをobject-orderの異なるone-line JSONと`Required=yes | Nullable=no`の`field_defaults`で表し、後続representationを同一fileの先行targetへ向けたbackward `same_as`にした。compact channelは`x-retrieval-unit: channel-file`でtargetを含むretrieval unitを明示する。両operation shardへ`Conventions`列を追加し、payload representationを持つoperationは`Data Representation`、依存を持たないoperationは`none`を選択した。`a-operation`のrequired Workflow closureは`none`で閉じる一方、四つのsupplemental Workflowと五つのdirect Workflow retrievalはwhole-CONVENTIONS fallbackを保持する。TDDの最初のREDはsource entries、compact forms、selective retrieval facts不在の4件で、fixture追加後はcandidate integration 17 / 17でGREENにした。実装中、restampのstaged final validationがCore validatorを直接呼びcompact syntaxを展開せず`DM-MSG-001/004`にする既存gapを発見した。承認された計画変更により、staged final validationをCoreを内包するcomplete validatorへ統一し、compact candidateを一時copyしてstale identityを明示的に作る回帰testを同じエラーのREDからGREENへ進めた。candidate source digestをmanifestへ再bindingし、同じexplicit manifestでfull / compactを個別restampした。最終実測は全786 tests中785成功 / 1 skip（Python 3.9未導入の既存環境skip）/ 0失敗、Core checkerは264 cases / 193 invalid、one-invalidity 193 / 193、未使用Core rule 0、coverage gap 0、complete checkerは16 pathsのcanonical equivalence、両profileのdry-runは`restamp required: no`となった。影響はcandidate JSON reuse projection、operation-level convention selector、restampのfinal validatorに限定し、document paths、Core corpus / publication gate、公開grammar、rule catalogは変更しない。次はTask 12のcomplete structuresに対するsource traceability checkpointへ進む。
