...
3. **Markdown-based** — docai uses structured Markdown and fenced code blocks so that examples and implementation guidance remain readable to an LLM and a human. docai must not be a YAML/JSON-only definition file.
...
```markdown
> docai: 1.0.0 | profile: full | generated: 2026-06-30 | generation_id: 20260630-abc123 | source: openapi.yaml (OpenAPI 3.1.1) | source_sha: abc123
```

- `docai` is the docai format version in `major.minor.patch` form(§3.1).
...
docai uses semantic `major.minor.patch` versions:
...
- `Summary` must add information beyond `Task`(key behavior, side effect, or distinguishing detail) — do not restate the task label. Keep it to one short sentence. A generator may apply a language- and tokenizer-specific budget, but docai does not define a UTF-8 byte limit because byte length is not a language-neutral measure of LLM token cost.
...
- `full` — the canonical detailed projection. It preserves all source information needed for a client to construct requests and interpret responses when that information is representable in docai. It is not a lossless serialization of the source schema.
...
- Put metadata information(docai format version, profile, generation date, generation ID, source, and source revision when available) at the beginning of **every file**, not only INDEX.md(see §3) — files are loaded individually.
...
- Before fixing generator defaults, benchmark representative tasks using both duplication and references. Compare total loaded tokens and correct-call rate for the target models; docai does not assume one strategy is universally cheaper.
...
- **Conversion is one-directional: source → docai.** docai is a generated artifact. The authoritative source(OpenAPI document, code, etc.) is the **maintenance source of truth**; docai is the client-implementation projection the LLM reads. Edit the source and regenerate docai — never the other way around.
- docai is not a lossless OpenAPI or JSON Schema representation and is not tied to one OpenAPI version. The `full` profile must preserve every source fact needed to call the API correctly that docai can represent. It must mark an unrepresentable client-relevant feature with `**unsupported**:` and direct the reader to its source location.
- The `compact` profile may intentionally reduce detail only under §3.4 and must point to the full set when both exist.
- docai does not replace OpenAPI. They coexist: OpenAPI or another authoritative source continues to serve validation, generation, and complete schema semantics; docai serves efficient LLM context.

## 9. Compliance Checklist

A document set is docai-compliant if:

- [ ] INDEX.md and CONVENTIONS.md exist
- [ ] The `docai` value uses `major.minor.patch`; no unknown non-`x-` structural text is present(§3.1)
- [ ] Every file(INDEX.md, CONVENTIONS.md, resources/, workflows/, webhooks/) begins with a metadata stamp containing `docai` / `profile` / `generated` / `generation_id` / `source`, and `source_sha` when available
