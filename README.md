...
DocAI HTTP has published `v1.0.0-rc.2` with a separate `1.0.0` conformance corpus at [`docai-http/fixtures/conformance/v1.0.0/`](docai-http/fixtures/conformance/v1.0.0/). A focused review found one remaining source-traceability blocker, so `v1.0.0-rc.3` is being prepared before final stable `v1.0.0`. This release work does not broaden the comparison claims above.
