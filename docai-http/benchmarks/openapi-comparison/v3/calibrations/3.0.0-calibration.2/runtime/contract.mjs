...
const CONTRACTS_FILE = path.join(PACKAGE_DIR, "contracts.json");
const TASK_PACKET_FILE = path.join(PACKAGE_DIR, "continuity", "tasks.json");
...
export function readTaskPacket(plan = readPlan()) {
  return validateBenchmarkTaskPacket(readContractPacket(TASK_PACKET_FILE), plan);
}

export function validateBenchmarkTaskPacket(packet, plan = readPlan()) {
  assertPlainJson(plan, "benchmark plan");
  assertPlainJson(packet, "task packet");
...
export function validateOutputContract(value, contractId) {
  const resolvedContractId = requireContractId(contractId);
  assertPlainJson(value, "output contract value");
  const contract = contracts.output_contracts[resolvedContractId];
  if (!contract) throw new Error(`unknown output contract ${resolvedContractId}`);
...
function requireContractId(contractId) {
  if (typeof contractId !== "string" || contractId.trim() === "") {
    throw new TypeError("contractId must be a non-empty primitive string");
  }
  return contractId;
}
