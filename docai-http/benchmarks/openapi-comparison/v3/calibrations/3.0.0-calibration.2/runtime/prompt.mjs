...
import { buildRequiredOutputText, readTaskPacket } from "./contract.mjs";
import {
  buildTaskContext,
} from "./context.mjs";
import {
...
export function buildPromptRecord(input) {
  assertPlainJson(input, "prompt input");
  const {
    plan = readPlan(),
    packet = undefined,
    run,
  ...
  const resolvedTask = canonicalTask(plan, resolvedPacket, task);
...
export function buildCalibrationPrompts(input = {}) {
  ...
  if (!isDeepStrictEqual(resolvedPacket, readTaskPacket(plan))) {
    throw new Error("prompt task packet identity must match the checked-in calibration packet");
  }
  const api = { id: plan.calibration.api_id };
  const tasks = new Map(resolvedPacket.tasks.map((task) => [task.id, task]));
...
export function buildCalibrationPromptRecords(plan = readPlan()) {
  return buildCalibrationPrompts({ plan });
}

export function promptMessages(record, input = {}) {
  validatePromptRecord(record, input);
  return [
...
export function renderedPromptText(record, input = {}) {
  return promptMessages(record, input)
...
export function buildPromptMetrics(records, input = {}) {
  assertPlainJson(records, "prompt records");
  assertPlainJson(input, "prompt metrics input");
  const { plan = readPlan(), packet = undefined } = input;
  validatePlanIdentity(plan);
  const resolvedPacket = packet ?? readTaskPacket(plan);
  const planIdentity = validateCanonicalPromptMatrix(records, plan, resolvedPacket);
  const rows = records.map((record) => promptMetric(record));
  return {
    metric_version: "1",
    benchmark_id: planIdentity.benchmark_id,
    plan_version: planIdentity.plan_version,
    methodology: {
...
export function buildPromptMetricsPacket(records) {
  return buildPromptMetrics(records);
}
...
function canonicalTask(plan, packet, task) {
...
function validateCanonicalPromptMatrix(records, plan, packet) {
  ...
  validatePlanIdentity(plan);
  if (!isDeepStrictEqual(packet, readTaskPacket(plan))) {
    throw new Error("prompt task packet identity must match the checked-in calibration packet");
  }
  ...
  records.forEach((record) => validatePromptRecord(record, { plan, packet }));
...
function validatePlanIdentity(plan) {
  assertPlainJson(plan, "prompt plan");
  validatePlan(plan);
