...
The current contents are an initial Compatibility Core corpus for draft `0.11.0`, a compact-profile candidate corpus, a workflow candidate corpus, a webhook candidate corpus, a non-JSON candidate corpus, a polymorphism candidate corpus, and a complete-surface candidate corpus for draft `0.12.0`. The complete conformance corpus that a stable release requires (README §9.1) does not exist yet; the core corpus is the pre-1.0 subset, and the complete-surface candidate corpus supports the broader pre-1.0 complete-generator-ready candidate label. The evidence gate for the `0.12.0` complete-generator-ready candidate label is tracked in [`../COMPLETE-GENERATOR-READINESS.md`](../COMPLETE-GENERATOR-READINESS.md).
