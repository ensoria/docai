...
- `complete-candidates/v0.11.0/focused/valid/` and `complete-candidates/v0.11.0/focused/invalid/` contain initial focused complete-candidate snippets for profile pairing, selective conventions, compact `same_as`, compact opaque fields, resource/workflow/webhook relations, non-JSON multipart, polymorphic variants, replacement `unsupported`, and grouped webhook variants.

Only the `core/` corpus is part of the current Compatibility Core evidence. The compact, workflow, webhook, non-JSON, polymorphism, and complete-surface candidate corpora do not declare those features ready for compatibility-preserving implementation. The non-JSON candidate corpus now includes CSV, XML, and SSE response fixture evidence, but CSV, XML, and SSE remain candidate-only. The polymorphism candidate corpus now includes tagged, untagged, and overlapping variant fixture evidence, but polymorphic variants remain candidate-only. The complete-surface candidate pair includes resources, workflows, and webhooks, and its focused snippets cover several complete-surface boundaries, but it still lacks full README section 9.1 focused coverage, complete checker coverage, and LLM task evaluation evidence. These fixtures do not declare selective convention loading, token-routing metadata, or other non-core structures ready for compatibility-preserving implementation.
