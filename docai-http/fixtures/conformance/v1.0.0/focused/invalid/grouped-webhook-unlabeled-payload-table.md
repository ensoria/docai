...
Expected: invalid complete conformance. Grouped webhook payload differences must use variant blocks, not an unlabeled common table.
...
| Field | Type | Presence | Nullable | Meaning |
|---|---|---|---|---|
| event | string | always | no | Always `subscription.cancelled` |
| subscription_id | string | always | no | Subscription ID |
| occurred_at | string | always | no | Composite deduplication timestamp |
