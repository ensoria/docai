...
Expected: valid complete conformance. A row with `code=unknown` may use an inline error label whose code token is the sentinel `unknown` when the row has the required `**unknown**:` marker.
