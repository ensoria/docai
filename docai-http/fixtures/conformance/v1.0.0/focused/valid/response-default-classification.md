...
Expected: valid complete conformance. A source `default` with exclusively error semantics is documented as a `default` row in `### Errors`; a source `default` that mixes non-error and error outcomes and cannot be split faithfully uses paired replacement markers in `### Response default` and `### Errors`.
...
**unsupported**: replaces Response default: mixed error and non-error outcome at fixtures/conformance/v1.0.0/source/complete-openapi.yaml#/paths/~1imports~1{id}/get/responses/default

### Errors

**unsupported**: replaces Errors: error branch is inseparable from the mixed default response at fixtures/conformance/v1.0.0/source/complete-openapi.yaml#/paths/~1imports~1{id}/get/responses/default
````
