...
Expected: valid complete conformance. Common error rows may use `Shape=none` when no body or caller-relevant headers exist, and `Shape=unknown` only with the required `**unknown**:` marker.
