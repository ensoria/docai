...
This directory is the stable conformance corpus for DocAI HTTP `1.0.0`.
...
found incomplete during focused review of `v1.0.0-rc.2`. `rc.4` completed
focused-fixture source-revision identity and checker coverage found incomplete
during review of published `v1.0.0-rc.3`; focused RC.4 review found no remaining
stable blocker.
...
This corpus covers the stable complete surface for `1.0.0`: full profile,
...
- `SOURCE-TRACEABILITY.md` records the source fixture audit and the decision not
  to include a public source-to-projection validator in `1.0.0`.
- `RC2-REVIEW-REGRESSION.md` maps the nine `rc.1` review findings to corrected
  files and deterministic regression evidence.
- `RC3-SOURCE-REVIEW.md` records the authoritative-source review after `rc.2`.
- `RC4-METADATA-REVIEW.md` records the completed focused metadata, checker
  rejection, and current-release review.
