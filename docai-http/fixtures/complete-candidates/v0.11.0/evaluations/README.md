...
Run `node tools/check-complete-evaluations.mjs` from the `docai-http/` directory, or `node docai-http/tools/check-complete-evaluations.mjs` from the repository root, to check that the task packet references existing files, covers all required task groups, has evidence strings in the selected retrieval context, and records a target model list for live evaluation.
