# Complete Candidate Token-Saving Notes

These notes describe compact-profile reductions used by the complete candidate example pair. They are candidate guidance only and do not promote compact output into the current Compatibility Core.

## Measurement Policy

Use tokenizer-specific measurements before a producer decides to emit a compact reduction. Token counts without an exact tokenizer identifier are not comparable, so this candidate corpus does not publish normative token counts.
