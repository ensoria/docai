# DocAI HTTP 0.12.0 Complete Candidate Fixtures

This directory contains the first candidate full/compact example pair for the complete generator implementation surface. It is not part of the `0.11.0` Compatibility Core and does not make DocAI HTTP stable.

This corpus is intentionally a pre-1.0 candidate example set, not the final complete conformance corpus required for a stable `1.0.0` release. It has focused complete-surface fixture coverage, corpus-specific checker coverage, required-target live LLM task evidence, token-load evidence, and scoped OpenAPI comparison evidence for the intended `0.12.0 Complete-generator-ready candidate` release label.
...
- `COVERAGE.md` records the complete-surface readiness evidence present for this candidate corpus.
- `TOKEN-SAVINGS.md` records compact-profile candidate reduction guidance.
- `evaluations/` contains the complete-candidate LLM evaluation task packet, local context metrics, live run records, and OpenAPI comparison baseline records.
