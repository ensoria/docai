...
Expected: invalid complete candidate. A source `default` response with exclusively error semantics belongs in `### Errors` as a `default` row, not as `### Response default`.
