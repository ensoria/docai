...
Promotion-scope decision: the first non-JSON candidate scope starts with `multipart/form-data` requests and now includes focused `application/x-www-form-urlencoded` request, raw binary upload/download, CSV response, and XML response candidates. Other non-JSON forms such as SSE remain outside this candidate scope until separate fixture and checker evidence exists.

Layout:

- `valid/full/` contains a full-profile candidate document set with multipart, form-urlencoded, raw binary, CSV, and XML endpoints.
