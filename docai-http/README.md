...
2. **Example-first** — Every non-empty request body and response body must include realistic concrete examples. Field tables supplement examples with constraints and presence rules. Authoritatively established body-less requests/responses must explicitly say `none`; missing body knowledge uses the `unknown` form in §3.4. In the `compact` profile, a request or response body may use `**same_as**:` instead of repeating a semantically identical earlier definition.
...
```markdown
> docai-http: 0.6.0 | profile: full | coverage: complete | knowledge: complete | generated: 2026-06-30 | generation_id: full-20260630-abc123 | projection_id: 20260630-abc123 | source: openapi.yaml (OpenAPI 3.1.1) | source_revision: sha256:abc123
```
...
DocAI HTTP 0.6.0 has no recursive-schema reference syntax. A directly or indirectly recursive request, response, error, parameter, or webhook shape cannot be represented by finite inline expansion. The generator must place `**unsupported**: recursive shape at <source location>` in the affected section and apply `coverage: requires-source`; it must not truncate the recursion at an arbitrary depth or invent a non-recursive shape. A representation strategy or a deliberate permanent exclusion for recursive schemas must be settled before 1.0.0(§9.1).
...
- [ ] CONVENTIONS.md uses every fixed heading in §3.3, including `HTTP Semantics`, in order; each common or validation error section that is neither `none` nor the valid whole-section `unknown` form begins with the required error table, every row maps to one valid shape or `none`, and each referenced shape includes its required body markers, representations, examples, field tables, and response headers
...
- [ ] Response headers the caller must read are documented(or confirmed as `none`), and each documented header states whether it is always or conditionally present; repeatable response headers define their field-line or list syntax, combination rule, order significance, and a concrete wire example
- [ ] Request fields and non-path parameters use `Required=yes|no|conditional`; every `conditional` row retains the column and states its exact condition in `Constraints / Meaning`; request field tables also specify nullability; response, endpoint-specific error, and webhook payload field tables specify presence and nullability; structured parameter fields specify requiredness relative to the containing parameter; a compact table may omit a uniform column or uniformly empty Meaning column only through a valid, token-saving `**field_defaults**:` marker
...
- [ ] Types use the defined nestable grammar without implying recursive schema support; formats such as RFC 3339 are written in the constraints or meaning column; every object container explicitly or conventionally defines whether and with what type it accepts additional properties
