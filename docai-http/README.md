...
> Specification version: 0.12.0 | status: Draft

> Publication label: Complete-generator-ready candidate; not stable, not 1.0.0, and not compatibility-stable.

This is a pre-1.0 complete-generator-ready candidate. It may be used as an implementation target for the complete draft surface evidenced by the fixture corpus in §9.1, including the required full profile, optional compact profile, workflows, webhooks, non-JSON representation rules, polymorphic variants, and the complete-surface unsupported/unknown forms covered by that corpus. It must not be presented as an initial stable release, a 1.0 release, compatibility-stable, or compatibility-preserving for future versions. The `0.11.0` Compatibility Core remains the narrower historical core implementation target; this `0.12.0` candidate broadens the evidenced surface but deliberately keeps the compatibility promise below stable. Stable compatibility guarantees begin with specification version 1.0.0. A later release that changes the compatibility scope must declare that scope, satisfy the matching fixture evidence requirements in §9.1, and use a publication label consistent with that evidence.
...
Readers that need to use a generated DocAI HTTP set do not need to load this entire specification. For task implementation, prefer the generated set's own retrieval path: `INDEX.md` → `CONVENTIONS.md` → selected resource/workflow/webhook files(§7.1). This `0.12.0` candidate includes fixture evidence for selective convention loading and compact retrieval in the complete-candidate corpus. Readers that intentionally target only the older `0.11.0` Compatibility Core should load the full profile and all of `CONVENTIONS.md`.
...
```markdown
> docai-http: 0.12.0 | profile: full | coverage: complete | knowledge: complete | generated: 2026-06-30 | generation_id: full-20260630-abc123 | projection_id: 20260630-abc123 | source: openapi.yaml (OpenAPI 3.1.1) | source_revision: sha256:abc123
```
