...
Before the first stable release, producers targeting APIs where recursive shapes are common should evaluate whether this unsupported-marker behavior is acceptable for implementation-ready projections. The specification should decide before stable 1.0 whether recursive schemas remain outside the supported scope or gain a finite representation. This decision is a 1.0 stability gate: a stable release should not leave recursive schema support in a state that is likely to require an immediate meaning-changing revision for common APIs. If recursive schemas must become representable before stable 1.0, the representation and fixtures should be added before declaring the format stable so early readers do not need an avoidable major-version migration.
...
Recursive schemas are deliberately outside the supported scope of this draft. Before stable 1.0, the specification should either keep that exclusion explicit for 1.0 or add a finite, self-contained representation with versioned fixtures. A stable release should not be published until this choice has been made explicitly, because changing it immediately after 1.0 could force an avoidable major-version migration for APIs with recursive shapes. A future version may define such a representation, but that addition must follow the compatibility rules in §3.1 and must include versioned fixtures before it is advertised as implementation-ready.
