...
> Specification version: 1.0.0 | status: Release candidate

> Publication label: 1.0.0 release candidate; not final Stable until the `v1.0.0` tag is published.

The current tagged public release is `v1.0.0-rc.4`, a fourth release candidate for the first stable DocAI HTTP contract. Review confirmed the authoritative source content added in `rc.3` and found that some focused conformance snippets omitted the stable source revision from their metadata stamps. The fourth RC completes that focused metadata identity and adds deterministic rejection coverage; it is awaiting focused review before final stable publication. It does not change the DocAI HTTP format, authoritative input content, or projected client contract. This release-candidate line may be used as an implementation target for the intended `1.0.0` stable conformance surface evidenced by the conformance corpus in §9.1, including the required full profile, optional compact profile, workflows, webhooks, non-JSON representation rules, polymorphic variants, and the complete-surface unsupported/unknown forms covered by that corpus. It must not be presented as the final stable publication until the repository publishes the final `v1.0.0` tag. The `0.11.0` Compatibility Core and `0.12.0` complete-generator-ready candidate remain historical implementation targets and supporting evidence. The intended `1.0.0` compatibility promise is limited to the normative README text, `fixtures/conformance/v1.0.0/`, and the self-contained `tools/check-conformance-fixtures.mjs`; candidate-only evidence paths and the boundary-isolation test harness do not create stable compatibility promises by themselves. A later release that changes the compatibility scope must declare that scope, satisfy the matching fixture evidence requirements in §9.1, and use a publication label consistent with that evidence.

Changes are recorded in the repository history and [CHANGELOG.md](CHANGELOG.md). Keeping detailed release history outside this README reduces tokens for readers that only need the current format rules. Before tagging or publishing a repository release, the specification version in this README, the metadata-stamp examples in this README, and the matching CHANGELOG.md entry for that same version must agree. README-visible changes must not remain only under `Unreleased` in the changelog for a tagged release. The readiness requirements are defined in §9.1.

### LLM Reader Quick Path (non-normative)

Readers that need to use a generated DocAI HTTP set do not need to load this entire specification. For task implementation, prefer the generated set's own retrieval path: `INDEX.md` → `CONVENTIONS.md` → selected resource/workflow/webhook files(§7.1). This `1.0.0` release candidate includes conformance-corpus evidence for selective convention loading and compact retrieval. Readers that intentionally target only the older `0.11.0` Compatibility Core should load the full profile and all of `CONVENTIONS.md`.
...
Code and Markdown snippets in this README demonstrate local syntax or placement rules unless they are explicitly described as a complete document set. They are not a substitute for the conformance fixtures required by §9.1, and a reader must not infer that omitted sibling sections, metadata, examples, or tables are optional in generated output.
...
```markdown
> docai-http: 1.0.0 | profile: full | coverage: complete | knowledge: complete | generated: 2026-06-30 | generation_id: full-20260630-abc123 | projection_id: 20260630-abc123 | source: openapi.yaml (OpenAPI 3.1.1) | source_revision: sha256:abc123
```
...
#### Pre-1.0 Compatibility Core

To make a pre-1.0 implementation target possible without pretending that the whole pre-1.0 surface is stable, a repository release may narrow its compatibility promise to a **Compatibility Core**. This is an intentional compromise to reduce future breaking changes: the basic full-profile HTTP documentation path can converge first, while token-saving structures and less-proven representation rules continue to collect fixture and implementation evidence.

A release that advertises a compatibility-preserving implementation target before 1.0.0 must state whether it targets only the Compatibility Core or the complete pre-1.0 surface. This is a repository publication label, not a `docai-http` metadata-stamp value. Unless the release explicitly expands the scope, the pre-1.0 Compatibility Core consists only of:

- Metadata stamp syntax for the `full` profile, including standard key order, value escaping, `coverage`, `knowledge`, generation/projection identity, optional `source_revision`, and `x-` extension placement rules.
- INDEX.md routing structure for resource files, including `# API Index`, `Endpoints`, `Workflows`, `Webhooks`, resource-file subsections, endpoint rows, `Task`, `Summary`, and `Also read`. Selective convention loading through the optional `Conventions` column remains outside the default Compatibility Core until fixtures prove that it preserves every applicable convention.
- CONVENTIONS.md fixed headings, ordinary convention prose/list/table content, and common error-shape definitions needed by endpoint error rows.
- Full-profile resource files for ordinary HTTP endpoints, including endpoint headings, purpose text, `Behavior`, `Request`, path/query/header/cookie parameters, represented JSON request and response bodies, response headers, endpoint-specific `Errors`, common and inline error-shape references, `Related`, `**deprecated**:`, and `**deviation**:`.
- `none`, `unknown`, and localized or core-unit replacement `unsupported` signaling for the core structures above, including the required `unsupported` fallback for recursive schemas.
- Canonical Markdown boundaries, table parsing, table-cell normalization, field paths, type grammar, response-status grammar, media-type spelling for JSON representations, and extension placement needed to validate the core structures above.

The following structures remain valid parts of this specification when a document set uses them, but they are outside the default pre-1.0 Compatibility Core unless a release explicitly promotes them with versioned fixtures: the `compact` profile and all compact reductions(`field_defaults`, `same_as`, compact opaque fields, and full/compact profile pairing), workflows, webhooks, non-JSON body rules, grouped webhook variants, polymorphic body variants, selective `Conventions` loading, optional token-routing `x-` metadata and INDEX columns, and replacement `unsupported` units that apply only to non-core workflow, webhook, compact, non-JSON, response-header-only, or other advanced structures.

Non-core structures may still use normative language in this specification; that language defines format compliance when the structure is used. It does not by itself make the structure part of a pre-1.0 compatibility promise. Before a non-core structure is promoted into the Compatibility Core, producers and validators should treat support for it as opt-in and should report non-core validation separately from core validation. After promotion, meaning-changing updates to that structure follow the compatibility rules of this section.
...
Pre-1.0 compatibility note: the default Compatibility Core(§3.1) includes only the `full` profile. The `compact` profile is intentionally kept outside that core until its token-saving rules have fixture and implementation evidence, because changing compact reductions after readers rely on them would create avoidable migration risk. A release may promote some or all compact structures into its compatibility scope only when it says so explicitly and publishes matching fixtures.
...
DocAI HTTP 1.0.0 has no recursive-schema reference syntax. Directly or indirectly recursive request, response, error, parameter, or webhook shapes are deliberately outside the intended 1.0.0 representable scope. This specification chooses the conservative first-stable-release path: recursive shapes remain unsupported for 1.0.0 unless a finite, self-contained representation and versioned fixtures are added before final stable `v1.0.0` publication. They cannot be represented by finite inline expansion. The generator must use the smallest applicable localized or replacement `**unsupported**:` form above and apply `coverage: requires-source`; it must not truncate the recursion at an arbitrary depth or invent a non-recursive shape.
...
Before final stable `v1.0.0` publication, producers targeting APIs where recursive shapes are common should evaluate whether this unsupported-marker behavior is acceptable for implementation-ready projections. If recursive schemas must become representable in 1.0.0, the representation and fixtures must be added before final stable `v1.0.0` publication so early readers do not need an avoidable major-version migration. After final stable publication, adding recursive-schema support should be treated as a post-1.0 compatibility decision under §3.1; if existing readers must understand the new representation to call those APIs correctly, it requires a new major version.
...
- A producer should define a target model, tokenizer, and token budget for its deployment and should keep each file within that measured budget together with the expected code context. This sizing guidance is advisory rather than a conformance condition because the deployment inputs are outside the DocAI HTTP document set. Do not use line count as a split criterion. Regardless of file size, producers must preserve the complete applicable client contract and must use the required `unknown` or `unsupported` handling instead of omitting information. Prefer task-oriented resource shards when a measured file would exceed the producer's configured budget.
...
1. Load the compact set's `INDEX.md` when the active compatibility scope includes the compact profile and a matching compact set exists; otherwise load the full set's `INDEX.md`. In the older `0.11.0` Compatibility Core path, load the full set.
2. Select the endpoint row by `Task`, `Method`, `Path`, and `Summary`.
3. Load the selected `CONVENTIONS.md` sections named by the optional `Conventions` column only when the active compatibility scope includes selective convention loading. In the older `0.11.0` Compatibility Core path, or when the column is absent, `all`, or not trusted by the reader, load all of `CONVENTIONS.md`. If a promoted or otherwise trusted column says `none`, load only the `CONVENTIONS.md` metadata stamp.
...
Conformance fixtures are separate versioned example files, not the illustrative snippets in this README. Repository release-candidate tags may use suffixes such as `v1.0.0-rc.1`, but generated fixture files declare the numeric DocAI HTTP version they test; for this release candidate, that version is `docai-http: 1.0.0`.

This repository's Compatibility Core fixture corpus for draft `0.11.0` is in [`fixtures/core/v0.11.0/`](fixtures/core/v0.11.0/), with its source OpenAPI fixture at [`fixtures/core-openapi.yaml`](fixtures/core-openapi.yaml). It is the historical core corpus, not the current release-candidate conformance corpus. Run `node tools/check-core-fixtures.mjs` from the `docai-http/` directory, or `node docai-http/tools/check-core-fixtures.mjs` from the repository root, to check the core fixture expectations. The checker is a fixture expectation checker for the published core corpus, not a full DocAI HTTP validator or a full source-to-projection validator.

Fixture requirements are staged to match the compatibility scope in §3.1. Before publishing a pre-1.0 release as the first Compatibility Core implementation target, the specification repository must publish at least one valid core full-profile document set and focused valid and invalid fixtures for every structure in the Compatibility Core. That core corpus (the valid full-profile document set together with the focused fixtures) must include INDEX.md, CONVENTIONS.md, at least one resource file, ordinary endpoint request/response examples, represented JSON bodies, parameters, response headers, common and inline errors, `none`, `unknown`, localized and core-unit replacement `unsupported`, recursive-schema `unsupported`, metadata escaping, extension placement, table parsing and normalization, field paths, type grammar, status ordering, generated-example validity, and coverage/knowledge states. Structural pieces such as INDEX.md, CONVENTIONS.md, and the resource file appear in the document set, while fine-grained syntax rules such as metadata escaping may be demonstrated only by a focused fixture. A Compatibility Core implementation target does not declare the `compact` profile, workflows, webhooks, non-JSON representations, selective convention loading, token-routing metadata, or other non-core structures ready for compatibility-preserving implementation.

Before publishing a pre-1.0 release as ready for the complete generator implementation surface, and before publishing `v1.0.0`, the specification repository must publish at least one complete valid full document set and its matching compact projection. Those example sets must include INDEX.md, CONVENTIONS.md, at least one resource file, one workflow file, and one webhook file. Across those example sets and the focused fixtures, the corpus must demonstrate selective conventions, common and inline errors, endpoint-specific handling that reuses a common error shape, non-JSON representation rules, grouped webhook payload variants, `unknown`, `unsupported`, response-header replacement `unsupported`, recursive-schema `unsupported`, workflow-section replacement `unsupported`, `field_defaults`, `same_as` with its discoverable intended retrieval unit, optional token-routing `x-` metadata when used, and both non-empty and omitted `Opaque fields` compact cases.

This repository's complete-generator-ready candidate corpus for draft `0.12.0` is in [`fixtures/complete-candidates/v0.12.0/`](fixtures/complete-candidates/v0.12.0/). It contains the complete full/compact pair, focused valid and invalid complete-surface fixtures, complete-candidate checker coverage, required-target LLM task evaluation records, deterministic token-load records, and OpenAPI comparison evidence. Run `node tools/check-complete-candidates.mjs`, `node tools/check-complete-evaluations.mjs`, and `node tools/check-openapi-comparison.mjs` from the `docai-http/` directory, or the same commands prefixed with `docai-http/` from the repository root, to check that corpus and its recorded evaluation evidence. This evidence supports the `0.12.0 Complete-generator-ready candidate` publication label, but it is still pre-1.0 candidate evidence and must not be described as stable or compatibility-preserving for future versions.

This repository's intended first stable conformance corpus for `1.0.0` is in [`fixtures/conformance/v1.0.0/`](fixtures/conformance/v1.0.0/). It copies the standard document content from the `0.12.0` complete-generator-ready candidate corpus into a separate versioned stable-boundary path, with `docai-http: 1.0.0` metadata stamps and stable conformance expectation labels. Run `node tools/check-conformance-fixtures.mjs` from the `docai-http/` directory, or `node docai-http/tools/check-conformance-fixtures.mjs` from the repository root, to check this corpus. The existing live LLM evaluation records, token-load records, and OpenAPI comparison records remain supporting evidence in `fixtures/complete-candidates/v0.12.0/` only while the conformance document content stays semantically identical to the evaluated candidate content; if the conformance document content changes, the release must decide which evaluations to refresh before `1.0.0`.

The source fixtures in the stable conformance corpus are traceability evidence, not a public source-to-projection validator contract. Before final stable `v1.0.0` publication, the repository does not require a reusable validator that proves every projected field was mechanically generated from the OpenAPI source; adding such a validator would require its own compatibility boundary. The stable corpus instead records source traceability in [`fixtures/conformance/v1.0.0/SOURCE-TRACEABILITY.md`](fixtures/conformance/v1.0.0/SOURCE-TRACEABILITY.md) and keeps `tools/check-conformance-fixtures.mjs` scoped to corpus-specific DocAI HTTP expectations.

The top-level publication label must remain consistent with the available evidence: a repository that publishes only the specification text, without the required core example set, is still a design-review draft even if the format rules themselves are internally consistent; a repository that publishes only the core corpus may be a Compatibility Core implementation target but not complete-generator-ready; and a repository advertised as ready for complete generator implementation must satisfy the complete-surface fixture requirements above. Stable publication additionally requires the stable conformance corpus below.

The repository must publish a versioned conformance corpus before the first stable release. The chosen release sequence is: publish `v1.0.0-rc.1` with this versioned conformance corpus; publish `v1.0.0-rc.2` after review found compatibility-scope and conformance-content corrections; publish `v1.0.0-rc.3` after focused review found incomplete authoritative source evidence in `rc.2`; and publish `v1.0.0-rc.4` after review found incomplete focused-fixture source identity in `rc.3`. Final `v1.0.0` may be tagged only after focused review confirms the corrected RC contract. If later review changes normative behavior, conformance content, checker expectations, or the compatibility boundary again, the repository must refresh affected evidence and publish another RC before stable. The conformance corpus must contain the complete example sets and focused valid and invalid fixtures for every canonical marker, table, table-cell normalization rule, representation class, representation media-type uniqueness rule, structured-parameter block, conditional-requiredness rule, field-path escape, object-openness rule, response-header presence and repetition rule, response-header replacement unit, workflow-section replacement unit, webhook payload rule, webhook grouping boundary, error-shape reference, endpoint-specific common-error deviation and suppression, error-time state recovery rule, compact error-shape field reduction, polymorphic form, metadata escape, optional token-routing `x-` metadata, coverage state, knowledge state, localized and replacement unsupported form, field default, compact opaque-field form, full/compact same-path profile pairing, exactly-null value representation, `same_as` same-kind and retrieval-unit requirements with discoverability, inline error labels whose code token is `unknown`, canonical boundary, extension placement, generated-example validity rule, and format-specific non-JSON requirement. It must also include directly and indirectly recursive-schema source fixtures whose generated projections demonstrate the required `unsupported` forms. Fixtures must declare the DocAI HTTP version they test and must not be silently changed after release; a meaning-changing fixture update follows the compatibility rules in §3.1. For the intended `1.0.0` stable boundary, structures covered by normative README text, [`fixtures/conformance/v1.0.0/`](fixtures/conformance/v1.0.0/), and `tools/check-conformance-fixtures.mjs` are the structures intended to become stable compatibility promises.

Recursive schemas are deliberately outside the supported scope of this release candidate and of the intended 1.0.0 stable contract. The first stable release should keep that exclusion explicit and cover it with fixtures, because adding a weak recursive representation immediately before 1.0 would be more dangerous than forcing an explicit source fallback. A future version may define a finite, self-contained recursive representation, but that addition must follow the compatibility rules in §3.1 and must include versioned fixtures before it is advertised as implementation-ready.
...
Publishing a draft for design review does not declare it implementation-ready or stable. A release advertised as a Compatibility Core implementation target must satisfy the core fixture requirements above and must not imply non-core compatibility. A release advertised as ready for complete generator implementation must satisfy the complete-surface fixture requirements above and demonstrate the replacement `unsupported` forms in its fixtures. A stable release additionally requires the versioned conformance corpus above. These publication labels are independent from whether an individual generated document set is implementation-ready under §3.
