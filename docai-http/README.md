...
```markdown
> docai-http: 0.5.0 | profile: full | coverage: complete | generated: 2026-06-30 | generation_id: full-20260630-abc123 | projection_id: 20260630-abc123 | source: openapi.yaml (OpenAPI 3.1.1) | source_revision: sha256:abc123
```
...
A document set is generated per profile: every file in a set carries the same `profile` value in its stamp. When the optional compact profile is generated, the two sets live in separate roots(for example, the full set in `docs/` and the compact set in `docs-compact/`), share one `projection_id`, and may have different `generation_id` values. An LLM implementing a client should load the compact set when it exists. It consults the matching full set for expanded examples, prose, or opaque response internals, not to discover omitted client-visible fields, statuses, constraints, or behavior. Each set's INDEX.md must state the other set's root on one line directly under the metadata stamp, using the fixed labels `Full set:` / `Compact set:`(for example, `Compact set: ../docs-compact/` in the full set), so an LLM that loaded one set can discover the other.
...
- Put metadata information(DocAI HTTP format version, profile, coverage, generation date, generation ID, projection ID, source, and source revision when available) at the beginning of **every file**, not only INDEX.md(see §3) — files are loaded individually.
...
- [ ] Every file(INDEX.md, CONVENTIONS.md, resources/, workflows/, webhooks/) begins with a metadata stamp in the fixed unescaped-` | `-separated key order of §3, containing `docai-http` / `profile` / `coverage` / `generated` / `generation_id` / `projection_id` / `source`, and `source_revision` when available; stamp values follow the escape-decoding rules and contain no unknown or trailing escape
...
- [ ] Each file's `coverage` matches the presence of `**unsupported**:` in its scope, and INDEX.md coverage summarizes the set; `requires-source` is treated as compliant but incomplete
...
- [ ] CONVENTIONS.md uses every fixed heading in §3.3, including `HTTP Semantics`, in order; each non-`none` common or validation error section begins with the required error table, every row maps to one valid shape or `none`, and each referenced shape includes its required body markers, representations, examples, field tables, and response headers
