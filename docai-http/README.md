...
> Specification version: 1.0.0 | status: Stable

> Publication label: Stable for the final `v1.0.0` publication.

This document is prepared for the final Stable `v1.0.0` publication from the reviewed `v1.0.0-rc.4` contract; the current tagged public release remains `v1.0.0-rc.4` until the final tag is published. Focused RC.4 review found no remaining stable blocker. The Stable release does not change the DocAI HTTP format, authoritative input content, or projected client contract from RC.4. It may be used as an implementation target for the `1.0.0` stable conformance surface evidenced by the conformance corpus in §9.1, including the required full profile, optional compact profile, workflows, webhooks, non-JSON representation rules, polymorphic variants, and the complete-surface unsupported/unknown forms covered by that corpus. The `0.11.0` Compatibility Core and `0.12.0` complete-generator-ready candidate remain historical implementation targets and supporting evidence. The `1.0.0` compatibility promise is limited to the normative README text, `fixtures/conformance/v1.0.0/`, and the self-contained `tools/check-conformance-fixtures.mjs`; candidate-only evidence paths and the boundary-isolation test harness do not create stable compatibility promises by themselves. A later release that changes the compatibility scope must declare that scope, satisfy the matching fixture evidence requirements in §9.1, and use a publication label consistent with that evidence.
...
Readers that need to use a generated DocAI HTTP set do not need to load this entire specification. For task implementation, prefer the generated set's own retrieval path: `INDEX.md` → `CONVENTIONS.md` → selected resource/workflow/webhook files(§7.1). This Stable `1.0.0` release includes conformance-corpus evidence for selective convention loading and compact retrieval. Readers that intentionally target only the older `0.11.0` Compatibility Core should load the full profile and all of `CONVENTIONS.md`.
...
DocAI HTTP 1.0.0 has no recursive-schema reference syntax. Directly or indirectly recursive request, response, error, parameter, or webhook shapes are deliberately outside the 1.0.0 representable scope. This specification chooses the conservative first-stable-release path: recursive shapes remain unsupported for 1.0.0 and cannot be represented by finite inline expansion. The generator must use the smallest applicable localized or replacement `**unsupported**:` form above and apply `coverage: requires-source`; it must not truncate the recursion at an arbitrary depth or invent a non-recursive shape.
...
Producers targeting APIs where recursive shapes are common must evaluate whether this unsupported-marker behavior is acceptable for implementation-ready projections. Adding recursive-schema support after 1.0.0 is a compatibility decision under §3.1; if existing readers must understand the new representation to call those APIs correctly, it requires a new major version.
...
Conformance fixtures are separate versioned example files, not the illustrative snippets in this README. Repository release-candidate tags may use suffixes such as `v1.0.0-rc.1`, but generated fixture files declare the numeric DocAI HTTP version they test; for this Stable release, that version is `docai-http: 1.0.0`.
...
This repository's stable conformance corpus for `1.0.0` is in [`fixtures/conformance/v1.0.0/`](fixtures/conformance/v1.0.0/). It copies the standard document content from the `0.12.0` complete-generator-ready candidate corpus into a separate versioned stable-boundary path, with `docai-http: 1.0.0` metadata stamps and stable conformance expectation labels. Run `node tools/check-conformance-fixtures.mjs` from the `docai-http/` directory, or `node docai-http/tools/check-conformance-fixtures.mjs` from the repository root, to check this corpus. The existing live LLM evaluation records, token-load records, and OpenAPI comparison records remain supporting evidence in `fixtures/complete-candidates/v0.12.0/`; later conformance-content changes must decide which evaluations to refresh before making evidence claims for the changed corpus.

The source fixtures in the stable conformance corpus are traceability evidence, not a public source-to-projection validator contract. Stable `1.0.0` does not require a reusable validator that proves every projected field was mechanically generated from the OpenAPI source; adding such a validator requires its own compatibility boundary. The stable corpus instead records source traceability in [`fixtures/conformance/v1.0.0/SOURCE-TRACEABILITY.md`](fixtures/conformance/v1.0.0/SOURCE-TRACEABILITY.md) and keeps `tools/check-conformance-fixtures.mjs` scoped to corpus-specific DocAI HTTP expectations.
